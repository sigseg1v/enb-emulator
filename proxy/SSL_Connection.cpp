...
SSL_Connection::SSL_Connection(SOCKET s, ServerManager& server_mgr, long ip_address)
    : m_Socket(s), m_ServerMgr(server_mgr), m_IpAddress(ip_address) {
...
void SSL_Connection::RunThread() {
    ...
    ssl_context = SSL_CTX_new(ssl_server_method);
    if (!ssl_context) {
    ...
    if (SSL_CTX_use_certificate_file(ssl_context, certf, SSL_FILETYPE_PEM) <= 0) {
        printf("SSL_CTX_use_certificate_file failed\n");
        m_ConnectionActive = false;
        return;
    }

    if (SSL_CTX_use_PrivateKey_file(ssl_context, keyf, SSL_FILETYPE_PEM) <= 0) {
    ...
    while (!g_ServerShutdown && m_SslConnectionThreadRunning) {
        //printf("SSL_Connection calling SSL_read\n");
        int bytes = SSL_read(ssl, recv_buffer, sizeof(recv_buffer) - 1);
        if (bytes < 0) {
            int error = SSL_get_error(ssl, bytes);
            if (error == SSL_ERROR_SYSCALL) {
                error = WSAGetLastError();
            }
            if ((error != SSL_ERROR_ZERO_RETURN) && (error != WSAECONNRESET)) {
            ...
            if (response) {
                ...
                ret = SSL_write(ssl, response, response_length);
                if (ret < 0) {
                    int error = SSL_get_error(ssl, ret);
...
*/
                ...
                if ((strlen(recv_buffer) + strlen(SERVER_HTML_PATH)) < MAX_PATH - 1) {
                    ...
                    if (legal && mime_type && open_type) {
                        ...
                        if (f) {
                            fseek(f, 0, SEEK_END);
                            size_t size = ftell(f);
                            if ((size > 0) && (size < MAX_SERVER_FILE_SIZE)) {
                                fseek(f, 0, SEEK_SET);
                                size_t actual = fread(m_Buffer, 1, size, f);
                                if (actual > 0) {
                                    response =
                                        HttpResult(response_length, m_Buffer, mime_type, actual);
                                }
                            }
                            fclose(f);
                        }
    ...
    if (!response) {
        ...
        static const char* error_404 = "HTTP/1.1 404 File Not Found\r\n"
...
char* SSL_Connection::AuthLogin(size_t* response_length, char* recv_buffer) {
	...
	*/
    ...
    if (username && password) {
        ...
        unsigned char* ip = (unsigned char*)&m_IpAddress;
        LogMessage("User '%s' logging in from IP: %u.%u.%u.%u\n", username, ip[0], ip[1], ip[2],
                   ip[3]);

        // TODO: This is a good place to check for "banned" IP addresses.
        // I don't think we should do "banned" IP addresses, lots of people have dynamic IP.
        // transmit ticket to server
        g_ServerMgr->m_UDPConnection->SendAccount(username, password, info);
    }

    return HttpResult(response_length, info, "text/plain");
}

char* SSL_Connection::HttpResult(size_t* response_length, char* data, char* content_type,
                                 size_t length) {
    if (length == 0) {
        length = strlen(data);
    }
...
char* SSL_Connection::SectorServer(size_t* response_length, char* recv_buffer) {
	...
	*/
    ...
    if (username && port && max_sectors && version) {
        ...
        sprintf(expected_version, "%d.%d", SECTOR_SERVER_MAJOR_VERSION,
        ...
        if (strcmp(version, expected_version) == 0) {
            ...
            if ((port_number >= 3500) && (max_sectors > 0)) {
                ...
                if (strcmp(temp, port) == 0) {
                    if (m_ServerMgr.m_SectorServerMgr.RegisterSectorServer(m_IpAddress, port_number,
                                                                           num_sectors, username)) {
                        strcpy(info, "Success=TRUE\r\n");
                    } else {
                        strcat(info, "The specified Sector Server IP Address and port are not "
                                     "pre-registered\r\n");
...
SSL_Connection::SSL_Connection(SOCKET s, ServerManager& server_mgr, long ip_address)
    : m_Socket(s), m_IpAddress(ip_address), m_ServerMgr(server_mgr),
      m_SslConnectionThreadRunning(false), m_ConnectionActive(false), m_ServerShutdown(false) {
