...
{
    short   Bitmask;            // 4 flags for condional fields
    long    GameID;
	long	TargetID;
    short   EffectDescID;
	char	*Message;
	...
	short	Duration;			// bitmask[2] 2 bytes (time is in milli seconds)
	float	TargetOffset[3];	// bitmask[3] 12 bytes
	short	OutsideTargetRadius;// bitmask[4] 2 bytes
	short	unused;				// bitmask[5] 2 bytes
