...
{
	//crashes here if sector connection has been terminated
	if (g_ServerMgr->m_SectorConnection)
	{
		...
		LogVMessage("<SERVER TO CLIENT TCP> ----> %04x [%04x]\n", opcode, bytes);
		...
		if (opcode == 0x1B)
		{
		long GameID = *((long*) &m_RecvBuffer[0]);
		LogMessage("Aux GameID: %d Size: %d\n", GameID, bytes);
		}
		*/
...
void Connection::QueuePosition(u8* packet, short &index, long game_id, float *pos, float *orientation)
{
	ConstantPositionalUpdate update;
	memset(&update, 0, sizeof(update));

	update.GameID = game_id;
	update.Position[0] = pos[0];
	update.Position[1] = pos[1];
	update.Position[2] = pos[2];
	...
	QueueResponse(packet, index, ENB_OPCODE_0040_CONSTANT_POSITIONAL_UPDATE, (unsigned char *) &update, sizeof(update));
}
...
void Connection::QueueAuxPacket(u8* packet, short &index, long game_id, char *name, float sig, u8 create_type, u8 sig_flags)
{
	...
	*((short *) &aux_data[8]) = length;
	...
	strncpy((char*)&aux_data[10], name, 200);
	if (create_type == 37)
	{
		u8 nav = (sig_flags & IS_NAV) ? 1 : 0;
		int i = 10 + length;
		aux_data[i++] = nav;
		*((float *) &aux_data[i]) = sig;
	}

	QueueResponse(packet, index, ENB_OPCODE_001B_AUX_DATA, aux_data, length+addon+6);

	u8 has_nav = (sig_flags & HAS_NAV) ? 1 : 0;
...
void Connection::ProcessAction()
{
	...
	case 18:	//gate button
		...
		//do not automatically request TCP link when gating - wait for the server handoff
		break;
