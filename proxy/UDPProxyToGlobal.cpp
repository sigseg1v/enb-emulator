...
{
	if (strlen((char *)msg) >= sizeof m_RecvBuffer)
	{
		LogMessage("Message too long in ValidAccount.\n");
	}
...
void UDPClient::ProcessAvatarList(unsigned char *msg, short len)
{
	if (strlen((char *)msg) >= sizeof m_RecvBuffer)
	{
		LogMessage("Message too long in ProcessAvatarList.\n");
	}
	m_global_account_rcv = true;
	memcpy(m_RecvBuffer, msg, len);
	m_ticket = (char*)m_RecvBuffer;
...
{
	m_ServerTCP = (0);
	Connection *global_connect = EstablishTCPConnection(ip_addr, SSL_LOCALCERT_LOGIN_PORT); //connect to Net7SSL global server port
...
Connection * UDPClient::EstablishTCPConnection(long ip_addr, short port)
{
	Connection *connection = (0);

	m_LoginComplete = false;

	//default to proxy server port on the server
	if (port == 0) 
	{
		port = PROXY_SERVER_PORT;
	}

	if (m_ServerTCP && m_ServerTCP->GetTCPPort() != port)
	{
		//shut down this link
	...
	connection = new Connection(s, *g_ServerMgr, port, CONNECTION_TYPE_SECTOR_SERVER_TO_PROXY);

	Sleep(50);

	connection->SendPlayerConnectionConfirm(m_PlayerID);

	LogMessage("Starting TCP login connection\n");
...
void UDPClient::KillTCPConnection()
{
	...
	if (m_ServerTCP)
	{
		...
		//m_ServerTCP->TerminateConnection();
		m_ServerTCP->SetTCPShutdownTime(GetNet7TickCount() + 5000); //5 seconds till TCP shutdown, plenty of time to accept the camera packets etc
		m_ServerTCP = (0);
	}
}
...
void __cdecl LoginCheckThread(void *Param)
{
	long cycles = 10; //up login wait time to 50 seconds
	login_thread_running = true;
	LogMessage(">> Commence Login check\n");
	//here we wait for 50 seconds, if login hasn't completed by then, tell the server to re-start login process
