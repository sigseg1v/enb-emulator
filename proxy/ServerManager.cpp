...
FILE* OpenLogFile(FILE* logfile, char* name) {
    ...
    if (!logfile) {
        ...
        sprintf(filename, "%s_%04d_%02d_%02d.log", name, systime.wYear, systime.wMonth,
                systime.wDay);
#else
        time_t now = time(NULL);
        struct tm* t = gmtime(&now);
        snprintf(filename, MAX_PATH, "%s_%04d_%02d_%02d.log", name, t->tm_year + 1900,
...
bool ServerManager::RegisterSectorServer(short first_port, short max_sectors) {
    ...
    snprintf(
        ...
        "\r\n",
        first_port, max_sectors, SECTOR_SERVER_MAJOR_VERSION, SECTOR_SERVER_MINOR_VERSION,
        g_DomainName);
