...
void SectorServerManager::AddSector(long sector_id, char* sector_name, char* system_name,
                                    char* parent_sector_name) {
...
bool SectorServerManager::SendPacket(long sector_id, short opcode, unsigned char* data,
                                     size_t length) {
    SectorServer* server = m_ServerList;
    while (server) {
        for (short i = 0; i < server->max_sectors; i++) {
            // Is this the correct server?
            if ((server->sector_assigned[i] == sector_id) && (server->tcp_connection[i])) {
...
bool SectorServerManager::RegisterSectorServer(unsigned long ip_address, short port_number,
                                               short max_sectors, char* username) {
    bool success = false;
    ...
    SectorServer* server = m_ServerList;
    while (server) {
        if ((server->ip_address == ip_address) && (server->port == port_number) &&
