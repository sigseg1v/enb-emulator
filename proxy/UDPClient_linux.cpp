...
bool UDPClient::OpenFixedPort(short port, long ip_addr) {
    ...
    if (::bind(m_Listen_Socket, (struct sockaddr*)&bind_addr, sizeof(bind_addr)) < 0) {
...
#endif
    if (::setsockopt(m_Listen_Socket, SOL_SOCKET, SO_RCVTIMEO, (const char*)&recv_timeout,
    ...
    if (!m_Unconnected) {
        if (::connect(m_Listen_Socket, (struct sockaddr*)&m_SockAddr, sizeof(m_SockAddr)) < 0) {
    ...
    if (::getsockname(m_Listen_Socket, (struct sockaddr*)&actual, &actual_len) == 0) {
        unsigned char* b = (unsigned char*)&server_ip;
        LogMessage("UDPClient: bound UDP %u (src) -> %u.%u.%u.%u:%d (%s peer)\n",
                   ntohs(actual.sin_port), b[0], b[1], b[2], b[3], (int)peer_port,
...
// ---------------------------------------------------------------------------
UDPClient::UDPClient(short port, short connection_type, long ip_addr, bool unconnected) {
    m_IPAddr = ip_addr;
    m_Port = port;
...
void UDPClient::RecvThread() {
    ...
    while (!g_ServerShutdown && m_recv_thread_running) {
        ...
        int received = UDP_RecvFromServer((char*)m_RecvBuffer, MAX_UDPC_BUFFER, src_addr, src_port);
        if (received <= 0) {
        ...
        if (m_Dtls) {
            ...
            net7::DtlsStep step = m_Dtls->Feed(peer, m_RecvBuffer, (size_t)received);
            if (!step.to_send.empty()) {
                RawSendToServer(step.to_send.data(), (int)step.to_send.size(), src_addr, src_port);
            }
            if (step.fatal) {
            ...
            for (const std::vector<uint8_t>& rec : step.app_data) {
                if (rec.size() < sizeof(EnbUdpHeader) || rec.size() > sizeof(m_RecvBuffer)) {
                    continue;
                }
...
void UDPClient::DispatchServerDatagram(int received) {
    if ((size_t)received < sizeof(EnbUdpHeader))
        return;
...
// report m_SockAddr.
int UDPClient::UDP_RecvFromServer(char* buffer, int size, long& src_addr,
    ...
    if (m_Unconnected) {
        ...
        rtn = ::recvfrom(m_Listen_Socket, buffer, size, 0, (struct sockaddr*)&src, &src_len);
        if (rtn > 0) {
            if ((long)src.sin_addr.s_addr != m_IPAddr) {
                unsigned char* b = (unsigned char*)&src.sin_addr.s_addr;
                LogMessage("UDPClient(Linux,unconnected): drop packet from %u.%u.%u.%u:%d (not "
                           "game server)\n",
...
void UDPClient::SendResponse(long player_id, short port, short opcode, unsigned char* data,
                             size_t length) {
    EnbUdpHeader* header = (EnbUdpHeader*)m_SendBuffer;
    header->size = (short)(length + sizeof(EnbUdpHeader));
    header->opcode = opcode;
...
void UDPClient::UDP_Send(short port, const char* buffer, int bufferLen) {
    ...
    if (m_Dtls) {
        short dest_port = (port == 0) ? (short)ntohs(m_SockAddr.sin_port) : port;
        uint64_t peer = net7::DtlsPeerKey((uint32_t)m_IPAddr, (uint16_t)dest_port);
        ...
        if (step.fatal) {
            ...
            LogMessage("UDPClient(Linux): DTLS send to port %d failed (%s); "
                       "datagram dropped\n",
                       (int)dest_port, m_Dtls->LastError().c_str());
            return;
        }
        if (!step.to_send.empty()) {
            RawSendToServer(step.to_send.data(), (int)step.to_send.size(), m_IPAddr,
    ...
    if (port == 0) {
        ...
        if (m_Unconnected) {
            sent = ::sendto(m_Listen_Socket, buffer, bufferLen, 0, (struct sockaddr*)&m_SockAddr,
                            sizeof(m_SockAddr));
        } else {
        ...
        sent = ::sendto(m_Listen_Socket, buffer, bufferLen, 0, (struct sockaddr*)&dst, sizeof(dst));
    }

    if (sent != bufferLen) {
        LogMessage("UDPClient(Linux): %s port %d failed: %zd/%d (%s)\n",
                   port == 0 ? "send" : "sendto", port, sent, bufferLen, strerror(errno));
    }
}
...
void UDPClient::RawSendToServer(const unsigned char* buffer, int bufferLen, long ip,
                                unsigned short port) {
    if (m_Listen_Socket == INVALID_SOCKET || bufferLen <= 0)
        return;
...
uint64_t UDPClient::ServerPeerKey(short dest_port_host) const {
    short p = (dest_port_host == 0) ? (short)ntohs(m_SockAddr.sin_port) : dest_port_host;
...
void UDPClient::DtlsKickHandshake(short dest_port_host) {
    ...
    short dp = (dest_port_host == 0) ? (short)ntohs(m_SockAddr.sin_port) : dest_port_host;
    uint64_t peer = ServerPeerKey(dp);
    if (m_Dtls->Established(peer))
        return;
...
void UDPClient::StartMVASKeepalive() {
    ...
    pthread_t tid;
    if (pthread_create(&tid, NULL, &LaunchUDPCKeepaliveThreadLinux, this) == 0) {
        pthread_detach(tid);
...
void UDPClient::SendServerKeepalive() {
    if (!m_ConnectionActive || m_PlayerID == 0)
        return;
...
bool UDPClient::ReadClientShipPosition(float pos[3], float heading[3]) {
    ...
    bool got = false;
    for (;;) {
        ssize_t n = ::recvfrom(m_PosIntakeSock, (char*)&dg, sizeof(dg), 0, NULL, NULL);
...
void UDPClient::SendPositionIfChanged() {
    if (!m_ConnectionActive || m_PlayerID == 0)
        return;
    if (m_SectorID >= 9999)
        return; // not in a real sector

    float pos[3], heading[3];
    if (!ReadClientShipPosition(pos, heading))
        return; // no live feed
...
char* UDPClient::SendTicket(char* ticket) {
    ...
    WaitForResponse((short)0, ENB_OPCODE_2002_TICKET, (unsigned char*)ticket, length);
...
char* UDPClient::CreateCharacter(GlobalCreateCharacter* create) {
    ...
    WaitForResponse((short)0, ENB_OPCODE_200B_CREATE_AVATAR, (unsigned char*)create,
                    sizeof(GlobalCreateCharacter));
