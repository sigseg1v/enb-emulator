...
class Connection {
    ...
    //////////////////////////////
public:
    Connection(SOCKET s, ServerManager& server_mgr, short port, int server_type,
...
public:
    ...
    void QueueResponse(unsigned char* packet, short& index, short opcode,
    ...
    void ForceLogout();
    void SendClientDamage(long target_id, long source_id, float damage, float modifier, long type,
                          long inflicted = 0);
    void SendObjectLinkedEffect(short bitmask, long UID, long effectID, short effectDID,
                                long effect_time);
    void QueueObjectLinkedEffect(u8* packet, short& index, short bitmask, long avatar_id, long UID,
                                 long effectID, short effectDID, long effect_time, long timestamp);
    void GlobalError(int Error); // Send Error
    void SendPlayerConnectionConfirm(long player_id);
    void SendLoginLink(UDPClient* client);
    void SetToProxyLink();

    void SetTCPShutdownTime(unsigned long time) {
        m_TcpShutdownCycle = time;
    }
...
public:
    ...
    void SendVersionResponse(long status); // opcode 0x01
                                           // opcode 0x04
    void SendCreate(int game_id, float scale, short asset, int type, float h = 0.0, float s = 0.0,
                    float v = 0.0);
    void QueueObjectCreate(u8* packet, short& index, int game_id, float scale, short asset,
                           int type, float h = 0.0f, float s = 0.0f, float v = 0.0f);
    ...
    // opcode 0x0a
    void QueueEffect(u8* packet, short& index, int player_id, int target_id, char* message,
                     short effect_type, long effect_id, long timestamp, long effect_time);

    void SendPointEffect(int effect_id, float x, float y, float z, short duration,
                         short effect_desc_id, float scale, float h = 0.0, float s = 0.0,
                         float v = 0.0);
    // opcode 0x0b
    ...
    void SendServerRedirect(long sector_id); // opcode 0x36
                                             // opcode 0x3a
    void SendServerHandoff(long from_sector_id, long to_sector_id, char* from_sector,
                           char* from_system, char* to_sector, char* to_system);
    ...
    // opcode 0x40
    void SendConstantPositionalUpdate(long game_id, float x, float y, float z,
                                      float* orientation = NULL);
    void SendFormationPositionalUpdate(long leader_id, long target_id, float x, float y, float z);

    // opcode 0x46
    void SendComponentPositionalUpdate(long object_id, PositionInformation* position_info,
                                       long timestamp = 0);
    // opcode 0x4a
    ...
    void QueueRelationship(u8* packet, short& index, long ObjectID, long Reaction,
                           bool IsAttacking);
    void SendCameraControl(long Message, long GameID);
    void QueueCameraControl(u8* packet, short& index, long Message, long GameID);
    void QueuePosition(u8* packet, short& index, long game_id, float* pos, float* orientation);
    void QueueAuxPacket(u8* packet, short& index, long game_id, char* name, float sig,
                        u8 create_type, u8 sig_flags);
    ...
    void SendClientChatEvent(int Unknown, int Unknown2, char* FName, char* LName, char* Channel,
                             char* Message);
    ...
    void ActivateBeamEffect(long player_id, long targetID, char* message, short effect_type,
                            long effectUID, long timestamp, short effect_time = 0);
    void QueueBeamEffect(u8* packet, short& index, long player_id, long targetID, char* message,
                         short effect_type, long effectUID, long timestamp, short effect_time = 0);
    ...
    void CreateTractorComponent(float* position, float decay, float tractor_speed, long player_id,
                                long article_id, long effect_id, long timestamp);
    void QueueTractorComponent(u8* packet, short& index, float* position, float tractor_speed,
                               long player_id, long article_id, long effect_id, long timestamp);
    void SendResourceContentsAUX(Object* obj);
    void SendResourceLevel(long target_id);
    void Dialog(char* Stringd, int Type);
    void SendObjectToObjectLinkedEffect(Object* target, Object* source, short effect1,
                                        short effect2, float speedup = 1.0f);
...
private:
    ...
    bool m_SectorTCPRequest;                // set as true if we've requested a TCP connection
...
private:
    ...
    pthread_t
        m_SendThreadHandle; // unused on the modern path; Connection.cpp's RunSendThread is a dead Win32-only branch
    Mutex m_Mutex;
