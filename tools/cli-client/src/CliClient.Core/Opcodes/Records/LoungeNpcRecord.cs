...
/// <summary>
/// 0x0052 LOUNGE_NPC. Wire shape (variable, up to ~10KB):
///   StationData (8 bytes): int32 StationType; int32 RoomCount.
///   RoomCount * StationRooms (28 bytes each):
///     int32 RoomNumber; int32 RoomStyle; float FogNear; float FogFar;
///     float FogRed; float FogGreen; float FogBlue.
///   int32 NumTerms.
///   NumTerms * StationTerms (16 bytes each):
///     int32 RoomNumber; int32 Location; int32 TermType; int32 Unknown.
///   int32 NumNPCs.
///   NumNPCs * StationNPC (variable, ~269 bytes each).
...
public sealed class LoungeNpcRecord : PacketRecord
{
    ...
    protected override void WriteFields(StringBuilder sb)
    {
        if (Payload.Length < 8)
        {
            Flag(sb, $"LOUNGE_NPC truncated -- {Payload.Length} bytes, expected >= 8");
            return;
        }
        int stationType = ReadI32LE(Payload, 0);
        int roomCount   = ReadI32LE(Payload, 4);

        FieldDec(sb, "StationType", stationType);
        FieldDec(sb, "RoomCount",   roomCount);
        ...
        for (int r = 0; r < roomCount; r++)
        {
            ...
            Field(sb, $"  Room[{r}]", $"num={roomNum} style={roomStyle} fog=({fogNear:0.##},{fogFar:0.##}) rgb=({fogR:0.##},{fogG:0.##},{fogB:0.##})");
        }

        if (off + 4 > Payload.Length) { Flag(sb, "truncated before NumTerms"); return; }
        int numTerms = ReadI32LE(Payload, off);
        FieldDec(sb, "NumTerms", numTerms);
        off += 4;
        ...
        for (int t = 0; t < numTerms; t++, off += TermSize)
        {
            ...
            Field(sb, $"  Term[{t}]", $"room={room} loc={location} type={termType}");
        }

        if (off + 4 > Payload.Length) { Flag(sb, "truncated before NumNPCs"); return; }
        int numNpcs = ReadI32LE(Payload, off);
        FieldDec(sb, "NumNPCs", numNpcs);
        // NPC bodies are large (AvatarData embedded); just report the count
        int remaining = Payload.Length - off - 4;
        Field(sb, "NPC data bytes", remaining.ToString());
    }
}
