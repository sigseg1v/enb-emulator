...
/// <summary>
/// 0x007F MANUFACTURE_SET_MANUFACTURE_ID. Wire shape: int32 mfg_id (4 bytes).
...
public sealed class ManufactureSetManufactureIdRecord : PacketRecord
{
    ...
    protected override void WriteFields(StringBuilder sb)
    {
        if (Payload.Length < 4)
        {
            Flag(sb, $"MANUFACTURE_SET_MANUFACTURE_ID truncated -- {Payload.Length} bytes, expected 4");
            return;
        }
        int mfgId = ReadI32LE(Payload, 0);
        FieldHex(sb, "ManufactureID", mfgId);
    }
}
