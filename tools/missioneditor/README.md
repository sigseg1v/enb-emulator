...
```
dotnet run --project tools/missioneditor/                # interactive
dotnet run --project tools/missioneditor/ -- --smoke     # CI smoke
```
...
- WinForms `DlgEditXml` → reused from `commontools` (already ported).
- WinForms `DlgSearch` → reused from `commontools`, configured against `Net7.Tables.missions`.
...
- **Talk-tree round trip.** The "Edit Talk Tree" button spawns `talktreeeditor` via `Process.Start` (same pattern as `station-tools`), but `talktreeeditor` does not return its result. Re-importing the edited XML back into `m_stage` needs a temp-file or stdout-pipe contract that isn't designed yet. Status bar surfaces this: "launched talktreeeditor (round-trip not wired)". Until then, talk-tree XML can be hand-edited via "Edit XML" on the toolbar (the whole mission XML).
