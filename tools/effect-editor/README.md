...
- **Every SQL statement is parameterised** through `commontools`'s `DB.Instance.executeQuery(sql, paramKeys[], paramVals[])`. The original built SQL by string concatenation (`"WHERE name LIKE '%" + textbox.Text + "%'"`, etc.) — roughly 20 textbook SQL-injection holes are silently closed by this port. Wire semantics are identical because all values go through MySqlConnector's placeholder binder.
...
smoke OK: all 5 effect-editor windows instantiated
```
