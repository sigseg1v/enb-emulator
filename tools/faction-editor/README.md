...
A toolbar/menu offers New / Save / Delete / Refresh against the live MySQL DB via `commontools` (`DB.Instance`).
...
Same pattern as `dataimport`: `ShutdownMode.OnExplicitShutdown`, the Login window's `Closed` event swaps `desktop.MainWindow` to the editor. Naive `ShowDialog().GetAwaiter().GetResult()` from the lifecycle callback deadlocks the dispatcher.
...
smoke OK: all 3 faction-editor windows instantiated
```

## What is *not* ported

- The original `Properties.Settings.Default` connection-info cache. The port persists last-used host/db/user via `commontools`'s JSON settings (same as the other Avalonia tools).
