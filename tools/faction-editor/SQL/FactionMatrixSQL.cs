...
namespace FactionEditor.SQL
{
    ...
    public sealed class FactionMatrixSQL
    {
        ...
        {
            // id is GENERATED BY DEFAULT AS IDENTITY -- omit it from the INSERT
            // and read it back with RETURNING (no LAST_INSERT_ID() in Postgres).
            // INSERT...RETURNING via executeQuery is recorded by the change
            // tracker.
            var lastInsert = DB.Instance.executeQuery(
                "INSERT INTO faction_matrix (\"faction_id\", \"faction_entry_id\", \"base_value\", \"reward_faction\") VALUES (@fid, @eid, 0, 0) RETURNING \"id\" AS id;",
                new[] { "fid", "eid" },
