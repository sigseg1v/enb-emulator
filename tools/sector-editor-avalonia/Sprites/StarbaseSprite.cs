...
// Ported from N7.Sprites.StarbaseSprite under Net-7 Entertainment's
...
{
    // Renders one starbase (`sector_objects` row of `type=12`): image +
    // signature/radar/exploration circles + name. Only one image
    // (stations.gif) regardless of radar state — matches the original.
    public class StarbaseSprite
    {
        ...
        public StarbaseSprite(PLayer layer, DataRow r, IPropertyHost pg, IGridSyncSink grid)
        {
            _pg = pg;
            ...
            // Single image regardless of radar; appearsInRadar still
            // tracks state for property panel round-trip.
            string imagePath = Mob.ResolveImage("stations.gif");
            starbaseImage = new PImage(imagePath);
            starbaseImage.X = (x - (starbaseImage.Width / 2)) / 100;
            starbaseImage.Y = (y - (starbaseImage.Height / 2)) / 100;

            float sigX = (x / 100) - ((sigDia / 2) - (starbaseImage.Width / 2));
            float sigY = (y / 100) - ((sigDia / 2) - (starbaseImage.Height / 2));
            float rrX  = (x / 100) - ((rrDia / 2)  - (starbaseImage.Width / 2));
            float rrY  = (y / 100) - ((rrDia / 2)  - (starbaseImage.Height / 2));
            float expX = (x / 100) - ((expDia / 2) - (starbaseImage.Width / 2));
            float expY = (y / 100) - ((expDia / 2) - (starbaseImage.Height / 2));

            var sigPen = new Pen(Color.Yellow,               3.0F);
            var rrPen  = new Pen(Color.LightGoldenrodYellow, 2.0F) { DashStyle = DashStyle.Dash };
            var expPen = new Pen(Color.LightYellow,          1.0F) { DashStyle = DashStyle.DashDotDot };
            ...
            starbaseImage.AddChild(sigCircle);
            starbaseImage.AddChild(rrCircle);
            starbaseImage.AddChild(expCircle);
            ...
            starbaseImage.MouseDown += Image_MouseDown;
            starbaseImage.MouseUp   += Image_MouseUp;
            starbaseImage.MouseDrag += Image_MouseDrag;
        ...
        public void updateChangedInfo(string propertyName, object _changedValue)
        {
            ...
            switch (propertyName)
            {
                ...
                case "Name":
                {
                    ...
                    var nameNode = (PText) starbaseImage.GetChild(3);
                ...
                case "IsCapShip":
                    dr["capShip"] = bool.Parse(changedValue); break;
                case "IsDockable":
                    dr["dockable"] = bool.Parse(changedValue); break;
                case "SoundEffect":
        ...
        private void ResizeCircleChild(int childIndex, float radius)
        {
            ...
            float x = starbaseImage.X;
            float y = starbaseImage.Y;
            var c = starbaseImage.GetChild(childIndex);
        ...
        {
            // Both 0 and 1 resolve to stations.gif (matches original);
            // appearsInRadar tracks the state for the property panel.
            if (type == 0) appearsInRadar = false;
            else if (type == 1) appearsInRadar = true;
            else return;

            string p = Mob.ResolveImage("stations.gif");
            float x = starbaseImage.X;
            float y = starbaseImage.Y;
            if (System.IO.File.Exists(p))
            {
                starbaseImage.Bitmap = new Bitmap(p);
                starbaseImage.X = x;
                starbaseImage.Y = y;
            }
        }
