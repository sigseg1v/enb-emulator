...
A small (260×380) window with one button per editor and a "Launch Net7" button at the bottom. Editor buttons spawn the sibling Avalonia projects from the current checkout via `dotnet run --project ../<name>/`. Editors that haven't been ported yet appear disabled with `(not yet ported)` suffixed.
...
Settings persist to `~/.config/FreyaTools/toolslauncher.json` on Linux and `%APPDATA%\FreyaTools\toolslauncher.json` on Windows (via `Environment.SpecialFolder.ApplicationData`).
...
smoke OK: all 2 toolslauncher windows instantiated
```
