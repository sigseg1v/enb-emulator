...
int main(int argc, char* argv[]) {
    ...
    */
    ...
    while (p->type != 0xFFFFFFFF) {
        if ((p->count > 0) && (!IsNested(p->type)) && (p->min_size == p->max_size)) {
            fprintf(outf, "%08X, %s, %d, %d, %d\n", p->type, p->text, p->count, p->min_size,
                    p->max_size);
        }
        p++;
    }
...
bool IsNested(DWORD type) {
    static DWORD nested[] = {W3D_CHUNK_MESH,
...
void ChunkAggregateInfo(BYTE* data, DWORD size) {
    ...
    ASSERT(size == sizeof(W3dAggregateInfoStruct) +
                       info->SubobjectCount * sizeof(W3dAggregateSubobjectStruct));

    char BaseModel[MAX_PATH];
    sprintf(BaseModel, "%s.w3d", info->BaseModelName);
    ProcessFile(BaseModel);

    W3dAggregateSubobjectStruct* subobject =
        (W3dAggregateSubobjectStruct*)(data + sizeof(W3dAggregateInfoStruct));
...
void ChunkHLodSubObject(BYTE* data, DWORD size) {
    ...
    W3dHLodSubObjectStruct* subobject =
        (W3dHLodSubObjectStruct*)(data + sizeof(W3dHLodArrayHeaderStruct));

    if ((isalpha(subobject->Name[0])) && (isalpha(subobject->Name[1])) &&
        (isalpha(subobject->Name[2])) && (isalpha(subobject->Name[3]))) {
