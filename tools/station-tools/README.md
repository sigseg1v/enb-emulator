...
Built on **.NET 10** + **Avalonia 11.2.3** + **MsBox.Avalonia 3.2.0** + the shared `commontools` library. Targets `net10.0` (no `-windows` suffix), so it runs on Linux without WINE.
...
- **Login** — shared from `commontools` (`CommonTools.Gui.Login`).
...
| `Helpers.cs` (`Utility`, `SQLData`) | `Utility` folded inline where used; `SQLData.ConnStr` dropped — uses `commontools`'s `LoginData.ConnStr` |
| `EditTalkTree.cs` + the entire embedded `TalkTreeEditor/` subdirectory | dropped — launches the standalone `talktreeeditor` via `dotnet run --project ../talktreeeditor/` |
| `Login.cs` + `Config.xml` roundtrip | dropped — uses `commontools`'s shared `Login` (persists creds via `LoginData`) |
| `SQLDataBase.cs` (private MySQL wrapper, sprintf-style SQL) | dropped — uses `commontools`'s parameterised `DB.Instance.executeQuery/executeCommand` |
| `StationSQL.cs` (empty stub in original) | dropped (was already dead in the WinForms tree) |
| MySql.Data 6.x reference | replaced by `MySqlConnector` (via `commontools`) |
...
- **Every SQL statement is parameterised** through `commontools`'s `DB.Instance.executeQuery(sql, paramKeys[], paramVals[])`. The original built SQL via string concatenation throughout `Main.cs` and `VenderTab.cs`; in particular, `ItemBrowse`'s `LIKE '%" + textbox + "%'` is the textbook injection hole. All ~40 call sites are now parameterised.
...
smoke OK: all 4 station-tools windows instantiated
```
