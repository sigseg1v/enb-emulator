...
| **12b** | Sql/ layer ported through `commontools`'s parameterised `DB.Instance` (kills the ~150 sprintf-style SQL sites in the original) | pending |
...
```
dotnet run --project tools/sector-editor/                # interactive (will show empty shell)
dotnet run --project tools/sector-editor/ -- --smoke     # CI smoke
```
