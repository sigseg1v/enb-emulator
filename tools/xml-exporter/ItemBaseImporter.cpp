...
void ItemBaseImporter::Normalize() {
    ...
    static int PriorityTable[] = {120, 121, 122, 100, 101, 102, 103, 110, 111, 140, 141,
                                  142, 150, 151, 152, 153, 160, 161, 162, 163, 170, 171,
                                  172, 173, 180, 181, 182, 183, 200, 201, 202, 203, 204,
                                  205, 206, 207, 208, 209, 210, 211, -1};
    ...
    for (int j = 0; j < m_ItemCount; j++) {
        ...
        if (m_Data[j].m_Category == -1 && m_Data[j].m_SubCategory >= 100 &&
            m_Data[j].m_SubCategory < 130) {
            printf("ItemID %d had invalid Category\n", m_Data[j].m_ItemTemplateID);
            m_Data[j].m_Category = m_Data[j].m_SubCategory / 10;
        }

        if (m_Data[j].m_SubCategory == -1 && m_Data[j].m_Category >= 10 &&
            m_Data[j].m_Category <= 12)
...
bool ItemBaseImporter::WriteFlatFile(char* FileName) {
    ...
    if (f) {
        if (fwrite(&m_ItemCount, 1, sizeof(m_ItemCount), f) == sizeof(m_ItemCount)) {
            if (fwrite(m_Data, 1, sizeof(ItemBaseStruct) * m_ItemCount, f) ==
                sizeof(ItemBaseStruct) * m_ItemCount) {
                success = true;
            }
        }
...
bool ItemBaseImporter::ReadFlatFile(char* FileName) {
    ...
    if (f) {
        if (fread(&m_ItemCount, 1, sizeof(m_ItemCount), f) == sizeof(m_ItemCount)) {
            ...
            if (fread(m_Data, 1, sizeof(ItemBaseStruct) * m_ItemCount, f) ==
                sizeof(ItemBaseStruct) * m_ItemCount) {
                success = true;
            }
        }
...
void ItemBaseImporter::ExportXML(char* FileName) {
    ...
    if (f) {
        ...
        for (int idx = 0; idx < m_ItemCount; idx++) {
            ...
            fprintf(
                f,
                "<ItemBase ItemTemplateID=\"%d\" Category=\"%d\" Subcategory=\"%d\" "
                "ItemType=\"%d\"\n\t\tGameBaseAsset=\"%d\"  IconBaseAsset=\"%d\" TechLevel=\"%d\" "
                "Cost=\"%d\"\n\t\tMaxStack=\"%d\" UseEffect=\"%d\" Flags=\"%d\">\n",
                m_Data[idx].m_ItemTemplateID, m_Data[idx].m_Category, m_Data[idx].m_SubCategory,
                m_Data[idx].m_ItemType, m_Data[idx].m_GameBaseAsset, m_Data[idx].m_IconBaseAsset,
                m_Data[idx].m_TechLevel, m_Data[idx].m_Cost, m_Data[idx].m_MaxStack,
                m_Data[idx].m_UseEffect, m_Data[idx].m_Flags);
            ...
            if (m_Data[idx].m_NumActivatableEffects > 0) {
                ...
                for (int j = 0; j < m_Data[idx].m_NumActivatableEffects; j++) {
                    fprintf(f, "\t\t<Effect flag1=\"%d\" flag2=\"%d\">\n",
                            m_Data[idx].m_ActivatableEffects[j].Flag1,
                            m_Data[idx].m_ActivatableEffects[j].Flag2);
            ...
            if (m_Data[idx].m_NumEquipableEffects > 0) {
                ...
                for (int j = 0; j < m_Data[idx].m_NumEquipableEffects; j++) {
                    fprintf(f, "\t\t<Effect flag1=\"%d\" flag2=\"%d\">\n",
                            m_Data[idx].m_EquipableEffects[j].Flag1,
                            m_Data[idx].m_EquipableEffects[j].Flag2);
