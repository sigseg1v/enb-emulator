...
static CatDat myCat[] = {
    "Beam",       10, 100, 14, "Beams",     10, 100, 14, "Projectiles", 10, 101, 16,
    "Missiles",   10, 102, 15, "Ammo",      10, 103, 10,

    "Devices",    11, 110, 11,

    "Reactors",   12, 120, 7,  "Engines",   12, 121, 6,  "Shields",     12, 122, 2,

    "Components", 50, 500, 13,

    "Ores",       80, -1,  13, "Resources", 80, -1,  13,
...
bool ItemBaseImporter::ConvertDase(char* Dase_DB, char* Dase_Host, char* Dase_User, char* Dase_Pass,
                                   char* Net7_DB, char* Net7_Host, char* Net7_User, char* Net7_Pass,
                                   int ClearTables) {
    ...
    if (ClearTables == 1) {
        ...
        if (!Net7_Query.run_query(ClearAmmo))
            return false;
        if (!Net7_Query.run_query(ClearBeam))
            return false;
        if (!Net7_Query.run_query(ClearProjectile))
            return false;
        if (!Net7_Query.run_query(ClearMissle))
            return false;
        if (!Net7_Query.run_query(ClearDevice))
            return false;
        if (!Net7_Query.run_query(ClearReactor))
            return false;
        if (!Net7_Query.run_query(ClearEngine))
            return false;
        if (!Net7_Query.run_query(ClearShield))
            return false;
        if (!Net7_Query.run_query(ClearManu))
            return false;
        if (!Net7_Query.run_query(ClearBase))
            return false;
        if (!Net7_Query.run_query(ClearManuBase))
            return false;
    ...
    if (!Dase_Query.run_query(DaseManufacturerQuery))
        return false;
    ...
    for (int i = 0; i < m_NumItems; i++) {
        ...
        m_ItemData->m_ItemFields[ITEM_FIELD_WEAPON_RANGE_I].iData =
            (int)m_ItemData->m_ItemFields[ITEM_FIELD_WEAPON_RANGE_I].fData;
        m_ItemData->m_ItemFields[ITEM_FIELD_EFFECT_RANGE_I].iData =
            m_ItemData->m_ItemFields[ITEM_FIELD_WEAPON_RANGE_I].iData;
        ...
        m_ItemData->m_ItemFields[ITEM_FIELD_ENGINE_SIGNATURE_I].iData =
            (int)m_ItemData->m_ItemFields[ITEM_FIELD_ENGINE_SIGNATURE_I].fData;
        ...
        m_ItemData->m_ItemFields[ITEM_FIELD_SHIELD_CAP_I].iData =
            m_ItemData->m_ItemFields[ITEM_FIELD_REACTOR_CAP_I].iData;
        m_ItemData->m_ItemFields[ITEM_FIELD_SHIELD_RECHARGE_F].fData =
            m_ItemData->m_ItemFields[ITEM_FIELD_REACTOR_RECHARGE_F].fData;
    ...
    for (int j = m_ItemStart; j < m_ItemCount; j++) {
        ...
        case 100: //Beams
            ...
            Net7_Q.AddData("damage_type",
                           m_ItemData->m_ItemFields[ITEM_FIELD_WEAPON_DAMAGE_TYPE_I].iData);
            ...
            Net7_Q.AddData("damage_100",
                           m_ItemData->m_ItemFields[ITEM_FIELD_WEAPON_DAMAGE_I].iData);
            Net7_Q.AddData("damage_200",
                           m_ItemData->m_ItemFields[ITEM_FIELD_WEAPON_DAMAGE_I].iData);
        ...
        case 101: //Projectiles
            if (m_ItemData->m_DefaultAmmo > 0) {
                sprintf(QueryString, "SELECT `RealName` FROM `items` WHERE `id` = %d",
                        m_ItemData->m_DefaultAmmo);
            ...
            Net7_Q.AddData("rounds_fired",
                           m_ItemData->m_ItemFields[ITEM_FIELD_ROUND_PER_SHOT_I].iData);

            Net7_Q.AddData("range", m_ItemData->m_ItemFields[ITEM_FIELD_WEAPON_RANGE_I].iData);
            Net7_Q.AddData("energy", m_ItemData->m_ItemFields[ITEM_FIELD_ENERGY_USE_F].fData);

            //Projectile launchers changed only in reload
            Net7_Q.AddData("reload_100",
                           m_ItemData->m_ItemFields[ITEM_FIELD_WEAPON_RELOAD_F].fData);
            Net7_Q.AddData("reload_200",
                           m_ItemData->m_ItemFields[ITEM_FIELD_WEAPON_RELOAD_F].fData);
        ...
        case 120: //Reactors
            ...
            Net7_Q.AddData("recharge_100",
                           m_ItemData->m_ItemFields[ITEM_FIELD_REACTOR_RECHARGE_F].fData);
            Net7_Q.AddData("recharge_200",
                           m_ItemData->m_ItemFields[ITEM_FIELD_REACTOR_RECHARGE_F].fData);
        ...
        case 121: //Engines
            ...
            Net7_Q.AddData("warp_drain_100",
                           m_ItemData->m_ItemFields[ITEM_FIELD_ENGINE_FREEWARP_DRAIN_F].fData);
            Net7_Q.AddData("warp_drain_200",
                           m_ItemData->m_ItemFields[ITEM_FIELD_ENGINE_FREEWARP_DRAIN_F].fData);
            ...
            Net7_Q.AddData("signature_100",
                           m_ItemData->m_ItemFields[ITEM_FIELD_ENGINE_SIGNATURE_I].iData);
            Net7_Q.AddData("signature_200",
                           m_ItemData->m_ItemFields[ITEM_FIELD_ENGINE_SIGNATURE_I].iData);
