...
# --- bounded deterministic field scatter (PB-25) -----------------------------
# Each fabricated field was emitted AT its matched nav-point's exact coordinates,
# so every field sat dead-centre on its waypoint marker ("stacked on waypoints").
# We instead offset the field CENTRE off the nav by a bounded, fully deterministic
# vector keyed on the stable (sector, nav) identity, so:
#   * the same inputs always produce the same SQL (generator output stays
#     reproducible and the committed seed is regenerable byte-for-byte), and
#   * the offset magnitude is a fraction of the field's own max_field_radius, so
#     the nav point stays INSIDE the field footprint (still reachable from the
#     waypoint) while the centre no longer coincides with the marker.
# We have NO sector-bounds data, so the offset is kept modest and the vertical
# (Y, EnB's up axis) component is scaled down to avoid pushing a field above/below
# the nav plane into a planet/ceiling. Navs are open space, so a sub-radius offset
# is low risk. This INVENTS a centre position within a bounded shell around an
# authoritative anchor; it does not invent which fields exist or what they yield.
SCATTER_MIN_FRAC = 0.35   # min offset as a fraction of max_field_radius
SCATTER_MAX_FRAC = 0.75   # max offset as a fraction of max_field_radius
SCATTER_VERT_FRAC = 0.25  # vertical (Y) component scaled by this vs horizontal


def scatter_offset(key: str, max_field_radius: float) -> tuple[float, float, float]:
    """Deterministic bounded (dx, dy, dz) offset for a field centre.

    `key` is the stable field identity (sector|nav); identical keys + radius
    always yield the identical offset. Direction is uniform on a sphere derived
    from a SHA-256 of the key; magnitude is in
    [SCATTER_MIN_FRAC, SCATTER_MAX_FRAC] x max_field_radius; the Y component is
    scaled by SCATTER_VERT_FRAC.
    """
    if max_field_radius <= 0:
        return (0.0, 0.0, 0.0)
    d = hashlib.sha256(key.encode("utf-8")).digest()
    u1 = int.from_bytes(d[0:8], "big") / float(1 << 64)   # azimuth fraction
    u2 = int.from_bytes(d[8:16], "big") / float(1 << 64)  # polar fraction
    u3 = int.from_bytes(d[16:24], "big") / float(1 << 64) # magnitude fraction
    theta = 2.0 * math.pi * u1
    cos_p = 2.0 * u2 - 1.0                 # uniform cos(polar) -> uniform on sphere
    sin_p = math.sqrt(max(0.0, 1.0 - cos_p * cos_p))
    mag = max_field_radius * (SCATTER_MIN_FRAC + (SCATTER_MAX_FRAC - SCATTER_MIN_FRAC) * u3)
    dx = mag * sin_p * math.cos(theta)
    dz = mag * sin_p * math.sin(theta)
    dy = mag * cos_p * SCATTER_VERT_FRAC
    return (dx, dy, dz)
...
def main():
    ...
    w("-- Source: the external reconstruct dataset (db/prospecting/prospecting.jsonl)")
    ...
    w("-- from the runtime. Each node is a type-38 sector_object placed near the")
    w("-- matched nav-point (offset by a bounded deterministic scatter so the")
    w("-- field centre is not dead-on the waypoint marker), plus its")
    w("-- harvestable + oretype +")
    ...
    for (rt_sector, navsoid), gap_items in sorted(PLACED, key=lambda kv: (kv[0][0], kv[0][1])):
        ...
        # offset by a bounded deterministic scatter so the field centre no longer
        # sits dead-on the waypoint marker (PB-25). Keyed on the stable
        # (sector, nav) identity so the same input always yields the same SQL.
        dx, dy, dz = scatter_offset(f"{rt_sector}|{navsoid}", float(prof["max_field_radius"]))
        pos_x = prow["x"] + dx
        pos_y = prow["y"] + dy
        pos_z = prow["z"] + dz
        w(
            ...
            f"38, {fnum(prow['scale'])}, {fnum(pos_x)}, {fnum(pos_y)}, {fnum(pos_z)}, "
