...
| `FreyaLauncher.exe` | Same host as the client (Win/WINE)                  | `tools/LaunchFreya/`                    | Patches client config / `authlogin.dll` offsets, runs `mkcert`, spawns the proxy + client. |
...
| ------------------------------ | ----- | -------- | ----------------------- | --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| **`LocalAuthRelay.ListenPort`** | **4180** | **TCP/HTTP (loopback)** | **`tools/LaunchFreya/Network/LocalAuthRelay.cs`** | **`authlogin.dll`** | **Plaintext HTTP terminator on `127.0.0.1`. Hardcoded `const` -- no setting moves it. Wraps upstream-bound traffic as TLS; see §Topology hops 1+2.** |
...
| `SECTOR_SERVER_PORT`           | 3501+ | TCP (legacy, unused) | nobody (server listeners commented out in `server/src/ServerManager.cpp`) | -- | Base port for the original direct per-sector TCP control plane. The server no longer binds it; the macro survives for the per-sector offset convention and for the proxy's framing. |
| `MASTER_SERVER_PORT`           | 3801  | TCP (legacy) | `proxy/` (`proxy/Net7.cpp:197`) | internal proxy listener | The original per-galaxy master handler ran here. The server side is gone; the proxy stands up a TCP listener on this port internally as part of its master-server role. |
| `GLOBAL_SERVER_PORT`           | 3805  | TCP (legacy) | `proxy/` (`proxy/ServerManager.cpp:61`) | internal proxy listener | The original cross-galaxy global control plane. As with 3801, the server side is gone; the proxy binds it internally. Live global control now runs over UDP on `UDP_GLOBAL_SERVER_PORT` (3810). |
...
### Client <-> proxy: Westwood RSA-512 + Westwood RC4
...
2. The proxy sends its 512-bit Westwood-format RSA public key (64-byte block, `WWRSA_BLOCK_SIZE`).
