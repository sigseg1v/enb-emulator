...
scripted login-and-do-X smoke runs, and the xUnit integration suite.
It is not a game client -- there is no rendering and no graphics. Plain
stdout, plain exit codes, plain test fixtures.

The project splits into two assemblies:

- `CliClient.Core` -- the protocol library (auth, handshake, codecs,
  log sinks, session/health logic). Everything the integration suite
  consumes lives here.
- `CliClient.App` -- the command-line front end (subcommands + the
  interactive REPL).
...
reimplementation of the retail Earth & Beyond protocol. Several rules
...
1. **Never modify the server to make the CLI client easier.** If the
   ...
   or first-hand documentation that proves the behaviour matches
   retail. See the "Server integrity rules" block in the top-level
...
| `--version` / `-v` | Print name + version and exit (rc 0) |
| `--smoke` | Print "ok: <name> <version>" and exit (rc 0) -- CI smoke target |
| `repl` (alias `start`) | Interactive REPL, see below |
...
from `OpcodeNameLookup`, which is seeded from every opcode declared in
`Opcodes.h` (~207 entries) -- so even an opcode the CLI has no typed
codec for still logs with its upstream symbolic name, e.g.
...
{"ts":"2026-05-24T12:34:56.131Z","dir":"in","op":54,"name":"ServerRedirect","bytes":10,"decoded":{"SectorId":422576128,"ServerEndPoint":"127.0.0.1:3503"}}
```

The `decoded` field is the typed codec's output when one is
registered; `RawPayload` (hex) when only a `NamedOpaqueCodec` is
in use; absent when the payload is empty. The integration suite
greps against these files.
...
sinks is left to callers (the integration suite wires them when it
needs to grep packet history).
...
- The standalone `send-chat` subcommand requires `--game-id` to
  match an avatar already attached to the session, because it does
  not run the avatar-selection handshake itself. The REPL `login` +
  `list` + `enter` commands do drive the global avatar handoff
  (`0x006D GlobalConnect` -> `0x006F GlobalTicket` ->
  `0x0070 GlobalAvatarList` -> `MasterJoin`), so end-to-end avatar
  selection is exercised there.
- Some enumerate / in-game workflows depend on the in-sector UDP
  opcode plane being complete. That plane is still in progress, so
  those flows are partial today; the CLI already drives the TCP
  planes (auth, global, master) plus the sector UDP/MVAS feed that
  is implemented.
- No GUI. There won't be one -- that's what the real client is for.
