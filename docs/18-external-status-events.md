...
| Player destroyed | `Player <name> (C##) was destroyed by <enemy> in <sector>.` |
| Jumpstarted | `Player <name> was jumpstarted in <sector>.` |

The wreck/jumpstart pair is handled specially by the sidecar (the only events
that are not a plain post): on a `player_destroyed` line the sidecar remembers the
Discord message id, keyed by the (server-unique) avatar name, in **process memory
only**. If the same player is `jumpstarted` within 30 minutes, the sidecar EDITS
that original message -- striking the wreck line through and appending the
jumpstart line -- rather than posting anew. A player who instead tows to a station
emits no jumpstart, so the wreck line correctly stands on its own. The correlation
state is deliberately non-durable: a sidecar restart forgets in-flight wrecks, and
an arriving jumpstart with no remembered wreck (towed, expired, or restarted) is
consumed without posting. `enemy` is `an unknown enemy` on environmental death.
...
CREATE TABLE external_status_events (
    ...
    kind        text        NOT NULL,   -- login|logout|levelup|broadcast|server_start|player_destroyed|jumpstarted
...
- `/notify list` -- show each kind (`login`, `logout`, `levelup`, `broadcast`,
  `server_start`, `player_destroyed`, `jumpstarted`) and whether it is currently relayed.
