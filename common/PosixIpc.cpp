...
bool FillSockaddrUn(const char* path, sockaddr_un& sa) {
    if (!path)
        return false;
    if (std::strlen(path) >= sizeof(sa.sun_path))
        return false;
...
PosixIpc::PosixIpc(const char* recv_path, const char* send_path)
    : m_recv_path(recv_path ? recv_path : ""), m_send_path(send_path ? send_path : "") {
...
bool PosixIpc::OpenRecv() {
    sockaddr_un sa;
    if (!FillSockaddrUn(m_recv_path.c_str(), sa))
        return false;
...
bool PosixIpc::WriteMessage(const char* message) {
    ...
    sockaddr_un sa;
    if (!FillSockaddrUn(m_send_path.c_str(), sa))
        return false;
    ...
    ssize_t sent = ::sendto(m_send_fd, message, len, MSG_NOSIGNAL, reinterpret_cast<sockaddr*>(&sa),
                            sizeof(sa));
    if (sent < 0) {
...
int PosixIpc::ReadMessage(char* buffer, std::size_t buf_size) {
    if (m_recv_fd < 0 || !buffer || buf_size == 0)
        return -1;
