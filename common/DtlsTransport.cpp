...
DtlsTransport::~DtlsTransport() {
    for (auto& kv : m_peers) {
        if (kv.second.ssl)
            SSL_free(kv.second.ssl); // frees the attached BIOs
    }
    m_peers.clear();
    if (m_ctx)
        SSL_CTX_free(m_ctx);
...
bool DtlsTransport::LoadServerCert(const char* cert_chain_path, const char* key_path) {
    ...
    if (SSL_CTX_use_certificate_chain_file(m_ctx, cert_chain_path) != 1) {
        m_last_error =
            std::string("load cert chain '") + cert_chain_path + "' failed: " + PopOpenSslError();
        return false;
    }
    if (SSL_CTX_use_PrivateKey_file(m_ctx, key_path, SSL_FILETYPE_PEM) != 1) {
        m_last_error =
            std::string("load private key '") + key_path + "' failed: " + PopOpenSslError();
        return false;
    }
...
DtlsStep DtlsTransport::ClientHandshake(uint64_t peer) {
    ...
    if (m_role != DtlsRole::Client) {
        out.fatal = true;
        m_last_error = "ClientHandshake on server transport";
        return out;
    }
    if (!Ok()) {
        out.fatal = true;
        m_last_error = "client CTX not ready";
        return out;
    }
    if (m_verify_host.empty()) {
...
DtlsStep DtlsTransport::SendApp(uint64_t peer, const uint8_t* data, size_t len) {
    ...
    if (!p) {
        out.fatal = true;
        m_last_error = "no association for peer";
        return out;
    }
...
void DtlsTransport::ForgetPeer(uint64_t peer) {
    ...
    if (it == m_peers.end())
        return;
    if (it->second.ssl)
        SSL_free(it->second.ssl); // frees rbio/wbio too
    m_peers.erase(it);
}
