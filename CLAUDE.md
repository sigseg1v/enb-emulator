...
The server is an OLD, questionable-quality implementation inherited from upstream, and the long-term plan is to rewrite it. **The server is ours to change** -- including adding behaviour the real Earth & Beyond server never had (new gameplay/automation systems, group logic, quality-of-life features) when that is the best and most reliable place to build a feature. **"Retail didn't do it this way" is NOT, by itself, a reason to refuse a server change, and you must not raise preservation/fidelity as a veto on adding new behaviour.** (Owner decision, 2026-07-05: the old "must match retail observable behaviour / server is for preservation only" gate was a temporary stance from when much of the code did not work; it has been REMOVED. Do not re-argue it.) Where a feature is most reliable -- server, proxy, or client -- put it there; the server is a legitimate and often the *best* choice because it holds authoritative game state.

Two things stay non-negotiable -- and both are ENGINEERING discipline, not preservation dogma:

1. **NEVER weaken the server's security posture for a tooling consumer's convenience.** This includes the CLI client (Phase S), the integration test suite (Phase T), the editor suite, packet-capture replay tooling, fuzzers, debug harnesses -- *anything*. Do not disable authentication checks, widen rate limits, accept malformed packets, bypass session-state guards, log secrets, or leak another player's private data, *because a tool finds it easier*. A tool wanting something just to make the tool's life easier is the tool being wrong, not the server. (Adding a NEW feature the owner asked for is not "tool convenience" -- this rule is about not lowering security bars, not about refusing features.)

2. **When you touch a WIRE FORMAT the real Win32 client must parse, get the bytes right so you do not crash the client.** This is why understanding-before-change still matters: for a packet the retail client already decodes, reproduce its EXACT byte layout (a primary source pins the format), and prove you understand it (CLI parse + test) BEFORE the server emits or accepts it. This is a *don't-crash-the-client* requirement, not a *don't-diverge-from-retail-behaviour* one: **reusing opcodes/packets the client already understands to drive NEW server behaviour is fully allowed** -- you only need format-fidelity for the bytes on the wire, never behaviour-fidelity for what the server chooses to do with them.

**When you change the server.** For a change that adds or alters a WIRE FORMAT (new/changed bytes the client parses), the following still apply so the real client keeps working:

**A. Primary-source proof of the BYTE FORMAT** when reproducing an existing retail packet (so our bytes match what the client expects). Not required when you are only reusing an already-understood packet to trigger new logic. Acceptable sources, in roughly decreasing order of weight:
