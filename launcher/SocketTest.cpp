...
bool engine_open_process(char* processwindowtitle) {
    ...
    if (TargetWindowHandle) {
        ...
        Process_Id =
            GetWindowThreadProcessId(TargetWindowHandle, (LPDWORD)&PID); //get a PROCESS number
...
bool engine_write_process(LPVOID lpBaseAddress, LPVOID lpBuffer, DWORD nSize) {
    ...
    if (g_EngineInUse) {
        ...
        if (g_ProcessHandle) {
            ...
            if (WriteProcessMemory(g_ProcessHandle, lpBaseAddress, lpBuffer, nSize,
                                   &BytesWritten) &&
                BytesWritten == nSize) {
...
bool engine_read_process(LPVOID lpBaseAddress, LPVOID lpBuffer, DWORD nSize) {
    ...
    if (g_EngineInUse) {
        if (g_ProcessHandle) {
            ...
            if (ReadProcessMemory(g_ProcessHandle, lpBaseAddress, lpBuffer, nSize, &BytesWritten) &&
                BytesRead == nSize) {
                SuccessCode = TRUE;
            }
...
void PatchClient() {
    ...
    unsigned char inject_buffer1[] = {
        ...
        0x75, 0x23,                                     //19
        0x8B, 0x48, 0x14, 0xBB, 0x58, 0x06, 0x21, 0x01, //this is the addr to write ptr to (25)
        0x89, 0x0B, 0x33, 0xDB, 0xEB, 0x15, 0x01, 0x90, 0x68, 0x28, 0x3C, 0xB7, 0x00, 0x50, 0x51,
        0xE8, 0x6A, 0xE2, 0x19, 0x00, 0x83, 0xC4, 0x10, 0xC2, 0x08, 0x00, 0x90, 0x8B, 0x48, 0x14,
        0x8D, 0x44, 0x19, 0x48, 0xE9, 0x30, 0x6E, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00};

    *((long*)&inject_buffer1[25]) = g_AddrStore;
    *((long*)&inject_buffer1[2]) = g_AddrStore;

    unsigned char inject_buffer2[] = {0xE9, 0x8E, 0x91, 0xEF, 0xFF, //jump to 888278
                                      0x90, 0x90};
    ...
    unsigned char inject_buffer4[] = {
        0x55, 0x33, 0xED, 0x89, 0x2D, 0xA8, 0xE5, 0xB6,
        0x00, //this resets the client coords position each time we change sectors
        0x5D, 0xC2, 0x04, 0x00, 0x90, 0x90, 0x90, 0x90};
    ...
    *((long*)&inject_buffer4[5]) = g_AddrStore;
    ...
    unsigned char inject_buffer5[] = {
        ...
        0x89, 0x35, 0xAC, 0xE5,
        0xB6, 0x00, //MOV DWORD PTR DS:[B6E5AC],ESI  //store gameID into 0x00b6e5ac (gameID flag)
        0x5E,       //POP ESI
        0xC2, 0x04, 0x00 //RETN 4
    };

    addr_off = g_AddrStore; //this addr is a fix temp store
    engine_write_process(
        (void*)addr_off, (void*)&zero,
        16); //make room for addr for position, orientation client gameID and also internal orientation update flag
...
void SendPositionIfChanged() {
    ...
    *((float*)&pos_data[24]) = *((float*)&buffer[32]);
    ...
    {
        SendResponse(ENB_OPCODE_1004_MVAS_SEND_POSITION_C_S, pos_data,
                     28); //send position and orientation change
...
void SetBroadcast(SOCKET socket) {
    ...
    setsockopt(socket, SOL_SOCKET, SO_BROADCAST, (char*)&broadcastPermission,
...
int UDP_SendToServer(char* buffer, long size) {
    ...
    if (sendto(m_Listen_Socket, buffer, size, 0, (sockaddr*)&lSockAddr, sizeof(lSockAddr)) !=
        size) {
        fprintf(stderr, "\nSend failed (sendto())");
    }
...
void UDP_Send(const char* buffer, int bufferLen) {
    ...
    if (buffsend = sendto(m_Listen_Socket, buffer, bufferLen, 0, (sockaddr*)&lSockAddr,
                          sizeof(lSockAddr)) != bufferLen) {
...
void SendThread(void* arg) {
    ...
    while (g_client_running) {
        ...
        if (CheckPosition()) { //OK, we've got a live position
...
bool StartENBClient(char* exe, char* cmd) {
    ...
    if (!engine_open_process("Earth & Beyond")) {
        ...
        success = CreateProcess(exe, cmd, NULL, NULL, TRUE, CREATE_DEFAULT_ERROR_MODE, NULL, NULL,
                                &si, &pi);
...
int main(int argc, char** argv) {
    ...
    SendResponse(ENB_OPCODE_1008_MVAS_LOGOFF_C_S, (unsigned char*)&gPlayerIndex,
                 sizeof(gPlayerIndex));
...
void LogMessage(char* format, ...) {
    ...
    fprintf(stderr, "%02d/%02d/%d %02d:%02d:%02d %s", timeinfo->tm_mon, timeinfo->tm_mday,
