...
CREATE TABLE `accounts` (
  ...
  -- Phase X: MySQL source kept in sync with db/postgres/seed.sql -- the
  -- production path is Postgres, but convert.sh re-derives the Postgres
  -- DDL from this file so the column shape must match. Stores an
  -- Argon2id PHC string (~96 chars). MD5 storage was removed in Phase X.
...
-- Records 
-- ----------------------------
-- No seed account ships here: a built-in account with a committed (hence
-- publicly known) password would be a standing superadmin backdoor on
-- every deploy. Provision at runtime instead -- tools/seed-dev-account/
-- seed-dev-account.sh for dev, registration/ChangePassword otherwise.
