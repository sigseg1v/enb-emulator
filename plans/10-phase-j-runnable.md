...
- [x] Linux proxy: framed opcode dispatch after RC4 handshake (replaces RC4-decrypt-and-discard).
      Status: done
      Touches: `proxy/Connection.cpp` (Linux RunRecvThread + SendResponse implemented — framed read of EnbTcpHeader, RC4-decrypt, payload read, RC4-decrypt, dispatch by m_ServerType; SendResponse builds header+payload, RC4-encrypts inline, sends), `proxy/ClientToMasterServer.cpp` (file-level WIN32 wall removed; ProcessMasterServerOpcode + SendServerRedirect now compile on Linux; HandleMasterJoin split into Win32 full impl and Linux option-b stub that logs avatar/sector + sends hardcoded ServerRedirect), `proxy/ClientToServer_linux_stubs.cpp` (new, `#ifndef WIN32` — logging stubs for ProcessGlobalServerOpcode + ProcessSectorServerOpcode so Linux proxy can SEE what real clients send next).
      Notes: Picked option (b) for HandleMasterJoin — UDP plane (UDPProxyMVAS / UDPClient) is still WIN32-walled and SendMasterLogin would have a chain of dependencies (`g_ServerMgr->m_UDPConnection`, `m_UDPClient`, `StartLoginTimer`) all stubbed-to-nothing on Linux. Hardcoded redirect points client at the proxy's own SECTOR_SERVER_PORT (3500) which doesn't have a listener yet — exposes the next gap honestly. ProcessGlobal/Sector remain logging stubs (Phase K work to port the 286 + 757 LOC of handlers). Integration test (5/5) still passes; proxy logs show `<client> MasterJoin avatar_id=1 ToSectorID=2147483647 FromSectorID=192226901 (Linux stub)` for the live replay — proving opcode dispatch fires where it previously dropped bytes silently. SendResponse is single-writer (called from the recv thread synchronously); needs Mutex once async senders (UDP plane / timers) come online.

- [x] Wire MySQL-backed AuthLogin into the Linux net7ssl listener.
      Status: done — `curl -k 'https://127.0.0.1:4443/AuthLogin?username=VectoR&password=go&serviceID=2184&version=2.5'` returns `Valid=TRUE\r\nTicket=VectoR-<rand>\r\n`; bad password returns `Valid=False\r\n`; `/sectorserver.cgi` returns `Success=TRUE\r\n`; `/touchsession.jsp?lkey=x` returns the chunked `Success` body; unknown paths return 404. All against the running MySQL container with seeded `net7_user.accounts` row.
      Touches: `login-server/Net7SSL/LinuxAuth.cpp` (new — 531 LOC, Linux-only TU; CC BY-NC-SA 3.0 header), `login-server/Net7SSL/SSL_Listener.cpp` (Linux accept branch now drives `SSL_read` → `HandleHttpsRequest` → `SSL_write` → `SSL_shutdown` inline instead of immediately closing), `login-server/Net7SSL/CMakeLists.txt` (pkg-config `mysqlclient` + link), `login-server/Dockerfile` (added `libmysqlclient-dev` to build stage, `libmysqlclient21` to runtime), `docker-compose.yml` (added `MYSQL_HOST/USER/PASS/DB` env vars to the `login` service).
      Notes: New TU is Linux-only (`#ifndef WIN32` file-level wall, matching the existing pattern). It uses libmysqlclient21 directly (not the vendored `mysqlplus` wrapper from `server/src/mysql/`) — a single `MYSQL*` connection guarded by `Mutex`, with `MYSQL_OPT_SSL_MODE=SSL_MODE_DISABLED` and `MYSQL_OPT_RECONNECT=true` (libmysqlclient21 prints a deprecation warning for the reconnect option — harmless, removed in 8.4). Ticket store is a 32-slot in-memory ring with the same 5-minute TTL as Win32 `AccountManager::IssueTicket`. Username/password go through a `[A-Za-z0-9_.-]` allowlist (username) / printable-ASCII-minus-quotes allowlist (password) before reaching the SQL literal — keeps SQLi surface at zero without dragging in `mysql_real_escape_string`. The Win32 `AccountManager`, `SSL_Connection` class, `Connection_B`, `ConnectionManager`, `UDPClient`, and the rest of the auth-server tree remain WIN32-walled — they're only needed for the in-game character/station/inventory flow, not the login round-trip.
      Limits / what's still WIN32-walled:
        - **Ticket handoff to game server is NOT wired**: the ticket is built and returned to the client, but the game server (which the client would present the ticket to) currently has no ticket-validation path on Linux either. The Win32 path pushes the ticket to the game server via `RegisterSectorServer` over a separate SSL connection on `SSL_LOCALCERT_LOGIN_PORT` (3807); that endpoint is not bound on Linux yet.
        - **`/who.cgi` is a 404 stub** — Win32 forwards to `PlayerManager::WhoHtml()`; PlayerManager is not ported.
