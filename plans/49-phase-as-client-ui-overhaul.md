...
| AS-17 Freya loot/salvage frame (LEFT-anchored readout) | in progress 2026-07-02 -- the native hulk loot window pops on the RIGHT, on top of the Freya party frame. New LEFT-anchored `mods/freya-hud/loot_frame.lua` repaints the contents Freya-style: per-slot **Slot / Name / Stack** (+ an icon tile, texture blit pending a live offset RE). Data via new DLL binding `enb.loot()` -> replays the three hulk cargo accessors (`0x00689ca0` ItemTemplateID / `0x00689d20` StackCount / `0x0068aaa0` TemplateAt) off the CONTAINER captured READ-ONLY from a naked ECX hook on `0x00689ca0` (`hooks::loot_container()`); each row `{slot,name,stack,tid,tmpl,asset}` + top-level `src` (container inventory-name, the hulk-vs-ship-inventory discriminator). `enb.loot_age()` gates freshness so a closed (possibly freed) container is never read. game.h (`addr::Cargo*` + `namespace cargo` offsets), hooks.cpp/.h (hook + accessors), lua_api.cpp (`l_loot`/`l_loot_age`). **READ path LIVE-VALIDATED 2026-07-02**: prospected asteroid reads "Copper Ore x5" byte-correct, `src=HarvestableResources`. Refinements shipped: header is just "Loot" (was "SALVAGE (src)"); a `src`-prefix filter drops the player's own holds (`Inventory*`), showing only lootable containers (harvest/hulk cargo). **TAKE path -- premise re-corrected 2026-07-02 back to opcode 0x0027 INVENTORY_MOVE (the 0x005D LootBuild attempt was WRONG for the mining/harvestable take and is abandoned)**: the native mining/loot take runs the UI inventory transaction (start `0x00690aa0` -> commit `0x00690d20`), and the commit builds an **INVENTORY_MOVE, wire opcode 0x0027** via in-place ctor `InvMoveBuild 0x00894ca0` (serializer `0x00894e30` stamps 0x27, len=size+4, six u32 fields: GameID, FromInv, FromSlot, ToInv, ToSlot, Num), pushed via `CmdSend 0x00728150`. For a prospected asteroid the server routes `FromInv=18 (0x12)` to `HandleInventoryMove` case 18 -> MineResource. New `enb.loot_take(slot)` replicates the commit exactly. **ROOT-CAUSE FIX 2026-07-02 (first wiring did not transmit -- "didnt loot fuckin shit")**: the send holder and GameID must come from the SClient that OWNS the open container -- `C = *(container + cargo::view_client==0x4c)`, GameID `= *(C + 0x1130)`, send through `C`'s Connection (`C + 0x1124`) -- NOT from the world-manager M. The first attempt fetched the sink via `M->vtbl[0x18](M)` (which returns M) and sent through M's connection; that silently fails to transmit an inventory command (verbs go through M, inventory does not). Now derived off the container per the native commit `FUN_00690d20`. Command object allocated with the CLIENT operator new (`0x004e9cb0`) and NOT freed (the Connection send path frees it through the matching heap; a DLL-heap/static buffer would be a cross-heap free -> crash). NOT a server/proxy change and not a new packet -- the client already sends this exact command on a native mining take; we only drive the same builder+sender. Clicking a Freya row's icon tile calls it (Lua wire live, guarded). game.h (`addr::InvMoveBuild`/`ClientOperatorNew` + `cargo::view_client`/`inv_move_size`/`inv_type_*`, `world::sub_client_gid`; removed the dead `sub_client_vtbl`), lua_api.cpp (`l_loot_take`). DLL rebuilt clean -Wall -Wextra + staged to `bin/enbmod.dll`. **NEEDS: one relaunch to load the new DLL, then live click-to-loot validation against a real prospected asteroid; native loot window stays functional underneath until confirmed. Then RE the icon texture offset off `row.asset` (read-only, no rebuild).** **REAL ROOT CAUSE FOUND + FIXED SERVER-SIDE 2026-07-02 (owner: "clicked it, nothing happened"): the client send path was CORRECT all along -- the 0x0027 FromInv=18 packet reaches `HandleInventoryMove` fine. The take was being silently DROPPED by my own AJ-3 destination-bounds check (Phase AJ security hardening, `PlayerConnection.cpp` HandleInventoryMove ~2626): it range-checked `InvMo.ToSlot` against the destination inventory's slot count, but the loot/mine source codes (`FromInv==18` mine/harvest, `FromInv==6` husk-loot) send `ToSlot==-1` and never index it -- their case bodies (case 18 -> MineResource / LootItem, case 6 -> LootItem) place the item via `CargoAddItem`, which auto-selects a free cargo slot. So `ToSlot==-1` failed the `>= 0 && < dst_slots` range check and was rejected BEFORE ever reaching the case body -- logged only via the dead `LogDebug` no-op, hence the silent "nothing happens". AJ-3 had whitelisted only the cargo/vault/vendor/manu auto-select combos (`auto_select_dest`) and so regressed every mine/husk-loot take.** **Fix:** added `bool dest_not_indexed = (InvMo.FromInv == 6 || InvMo.FromInv == 18);` to the guard so those two source codes skip the destination range check (their wire `ToSlot` carries no meaning). This is a pure REGRESSION REPAIR -- it restores pre-AJ-3 behaviour and does NOT weaken security (ToSlot is never used as an index for 6/18, so no bounds can be violated); it only stops rejecting a take the real server always accepted. Any residual failure is now VISIBLE (CheckMiningConditions / range gates in MineResource `SendMessageString` to the client), never silent. TEMP debug (rx-opcode / InvMove-entry / case-18 dump in PlayerConnection.cpp, MiningCond dump in PlayerSkills.cpp) all reverted; server + DLL rebuilt clean; client relogged as devuserje (the LV75 miner). **NEEDS: owner clicks a loot row on an in-range prospected asteroid on devuserje to confirm live.** CV-AS-LOOT + new CV-BA-LOOT (plans/29). **LOOT-FRAME "no panel on first resource after login" FIXED 2026-07-03 (owner-reported + owner-confirmed live "works"):** the loot capture used a single global latch (`g_loot_container`) set by the `CargoTemplateID` hook, which fires for EVERY cargo grid the client paints -- including the player's own always-visible `Inventory.*` holds, which repaint every frame. Live probe with a resource selected showed the latch stuck on `Inventory.Equipped` 10/10, so `loot_frame.lua`'s `src`-prefix filter correctly suppressed the panel; the harvestable grid does paint (a target SWAP made the panel appear) but lost the latch race to the inventory grid. **Owner's theory ("re-evaluate the selected target every second") was wrong** -- the Lua already re-evaluates every tick; the variable was the DLL latch, not the Lua cadence. **Fix (hooks.cpp `notify_loot_container`):** reject any container whose inventory-name begins "Inventory" (case-insensitive, direct byte compare bounded by a `mem::readable` check -- no `mem::cstr` setjmp on the per-frame render-thread hot path) from taking the latch, so a targeted hulk/resource container holds it once the harvestable grid paints once. Real loot containers ("Cargo" hulk / "Harvestable..." resource) still latch; an unreadable name latches as before. DLL rebuilt clean -Wall -Wextra; loot frame now shows on first resource select after login. |
