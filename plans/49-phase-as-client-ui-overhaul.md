...
- `[x]` **Target action buttons always visible.** Owner: the contextual
  target-action buttons (Follow / Dock / Register / Loot / ...) that sit above
  the native target frame were only visible on hover. Root cause (extends the
  AS-13 (4) finding): the game rests that transient CHROME_VT cluster at draw-gate
  **0x500** (Visible|inLayout, Enabled CLEAR) and only ORs in Enabled (0x200 ->
  0x700) on mouse-over. Fix in `hide-ui/native_hud_hide.lua`: each tick, walk the
  transient FRONT leaves (the `base` count `map_panels()` already returns) and for
  any leaf whose flags == `0x500` (`REST_DISABLED`, i.e. masked to the 0x700 gate)
  OR in `ENABLE_BIT` so it draws at rest. Gated behind a `show_target_buttons`
  config flag. Verified live: target leaf went `0x6501 -> 0x6701` and the gold
  buttons render at rest (no hover) on a mob target. Owner: "Later we will improve
  them but this will be fine for now" -- styling pass deferred. Pure Lua, hot-reloaded.
...
| AS-14 res-scaled layout + always-on target buttons | done (owner ask 2026-06-28) -- new `lib/freya_hud.lua` `H.sx/H.sy/H.smul` scale helpers (0 at the 1280x960 baseline, full at 2560x1440, linear in between); owner-tuned 1440p nudges applied per element (target frame in 32 + bars +6h + value 1.5x; action bar/player card up 20; play card right 32; disc card up 20; chat up 12/right 32; micro-menu right 32/down 16), none compounding. Target action buttons (Follow/Dock/Loot/...) forced always-visible: `hide-ui` re-asserts ENABLE_BIT each tick on transient front leaves resting at the 0x500 gate (game only sets it on hover) -> 0x700 draws at rest. Pure Lua, hot-reloaded + verified live at 2560x1440 |
