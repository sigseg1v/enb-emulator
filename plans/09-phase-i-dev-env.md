...
- [ ] **Rename the MySQL-named DB layer to Postgres** (owner-requested 2026-06-02, to stop the filenames implying MySQL). The server's DB layer is already a libpqxx (Postgres) shim, but the names still say "mysql": `server/src/mysql/mysqlplus.{cpp,h}` (the libpqxx-backed 7-class wrapper), the `USE_MYSQL_SECTOR` / `USE_MYSQL_ACCOUNT_DATA` / `USE_MYSQL_STATIONS` / `USE_MYSQL_ITEMS` macros in `Net7.h`, the `*SQL.cpp` loaders (`MOBDatabaseSQL.cpp`, `FactionDataSQL.cpp`, `SectorContentSQL.cpp`, `SkillsDatabaseSQL.cpp`), and the `g_MySQL_Host/User/Pass` globals. Scope: rename dir `server/src/mysql/` -> e.g. `server/src/db/`, the `mysqlplus` wrapper -> a neutral name, the `USE_MYSQL_*` macros -> `USE_PG_*` (or just delete them -- they are unconditionally defined, so they are dead toggles per the no-dead-code rule), update every `#include "mysql/mysqlplus.h"` site + CMakeLists, and rename the `g_MySQL_*` connection globals. Pure rename / no behaviour change; keep the libpqxx dialect translation intact. One mechanical pass, green build before and after. Do NOT rename the vendored MySQL Connector headers under `server/third_party/` (we consume those) and do NOT touch `db/mysql/` (the original upstream MySQL dumps, kept for reference).
