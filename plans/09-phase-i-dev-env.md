...
- [ ] **(follow-up) Delete the dead non-SQL fallback path + `USE_PG_*` macros.** Now that `SQL_ENABLE` (and thus `USE_PG_SECTOR/ACCOUNT_DATA/STATIONS/ITEMS`) is unconditionally defined, the `#ifndef USE_PG_*`-gated alternative implementations are unreachable: `ItemBaseParser.cpp` (whole body `#ifndef USE_PG_ITEMS`) and `SectorContentParser.cpp` (whole body `#ifndef USE_PG_SECTOR`) compile to nothing today. Remove them, drop the `#ifndef`/`#error` sanity asserts, collapse the `#ifdef USE_PG_*/#else` regions in `AccountManager.{cpp,h}` + `StaticData.h` to their always-taken `#ifdef` branch, then delete the four `USE_PG_*` defines from `Net7.h` (collapsing `SQL_ACCOUNT_STRING` to its constant value). Verify each file's preprocessor boundary by hand (the `AccountManager.cpp` 121..1039 `#ifdef/#else` region is the risky one); green docker build before and after. Optionally fold `SQL_ENABLE` itself away in the same pass (also unconditionally defined; gates `PlayerConnection.cpp:9144` + `Net7.cpp:295`).
