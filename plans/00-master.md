...
| M | Eliminate Win32 from server-native code | [13-phase-m-win32-elimination.md](13-phase-m-win32-elimination.md) | in progress — bulk done (all 3 compat/ dirs deleted; mailslot→AF_UNIX rewrite for server+login MailslotManager; CreateMutex→flock via net7ipc::SingleInstance; _beginthreadex→pthread_create for every Linux-active call site; OpenSSL opensslconf.h Win32-gating patch removes the last <windows.h> drag; CMakeLists references stripped; 20/20 tests green; docs + decisions log updated). **Remaining:** vocabulary sweep — ~58 live Sleep/GetTickCount/SOCKET uses now behind inlined umbrella-header typedefs; rewrite lands alongside Phase K TU work, not as a separate pass. | 2026-05-23 | 2026-05-23 |
