...
| N | `mysqlplus.cpp` → libpqxx rewrite (Phase C continuation) | [14-phase-n-libpqxx-rewrite.md](14-phase-n-libpqxx-rewrite.md) | complete — Wave 1 (wrapper-only): 7-class API reimplemented over libpqxx 7.x preserving every public signature; opaque-handle pattern keeps libpqxx headers out of `mysqlplus.h`; per-row `char**` cache preserves `sql_var_c` lifetime contract; backtick→double-quote dialect translation; `mysql_escape_string` shim retained for SaveManager. Server CMakeLists swapped to `PkgConfig::LIBPQXX + PostgreSQL::PostgreSQL`; Dockerfile drops libmysqlclient21+libmysqlclient-dev; rebuilt binary 13.7 MB linking libpqxx-7.8+libpq.so.5 only. All ~25 `*SQL.cpp` DAOs compile unchanged; 8/8 live integration suite green. Wrapper round-trip test (`tests/db/mysqlplus_wrapper_test.cpp`) added — 3 env-gated cases incl. hostile-literal round-trip. **Wave 2 (parameterised `execute()` API + 36-site DAO call-site sweep + `tools/sql_injection_audit.sh` + `SafeUsername`/`SafePassword` retirement) deferred to a follow-up phase — wide call-site scope.** | 2026-05-24 | 2026-05-24 |
