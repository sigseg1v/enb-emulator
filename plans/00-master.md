...
| R | Extract `common/` for shared protocol headers | [18-phase-r-common-headers.md](18-phase-r-common-headers.md) | in progress (Wave 1 + Wave 2 done — wire-format wins complete: Opcodes.h + Mutex.h + WestwoodRC4.h + WestwoodRSA.h + PacketStructures.h + Packing.h + Ports.h all consolidated, all 3 binaries link [163/16/8], 22/22 tests green incl. 8/8 live integration. Caught two real wire-format drifts during the merge: server/login MasterJoin still used `long` (108B on Linux vs 64B on wire) — backported Phase K int32_t fix; proxy's SECTOR_SERVER_PORT was 3500 vs server/login's 3501 — split into PROXY_LOCAL_TCP_PORT/SECTOR_SERVER_PORT. Wave 3 backlog (cleanup of Globals.h/CircularBuffer.h/cmdcodes.h/Net7Types.h) deferred — those aren't wire-format. Doc updates (CLAUDE.md, docs/02-architecture.md) outstanding before close.) | 2026-05-23 | 2026-05-23 |
