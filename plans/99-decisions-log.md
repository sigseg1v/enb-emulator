...
## 2026-05-27 — Phase V: client isolation, mkcert, neutral hostnames
- **The client only ever talks to `localhost`.** Public-server hostnames are out of the client tree. The launcher always patches `Network.ini` / `Auth.ini` / `rg_regdata.ini` to point at the local hostname (`Launcher.LocalHostname = "localhost"`), regardless of `UseLocalCert`. The proxy is the only thing that contacts an upstream.
- **Upstream host is runtime-configurable, not baked in.** Proxy reads `NET7_UPSTREAM_HOST` env var or `/UPSTREAM:<host>` CLI flag. Default is empty (no upstream — outbound flows that need one must guard and skip). The launcher passes the user-selected `Setting.Hostname` through as the env var when spawning `Net7Proxy.exe`.
- **Cert trust uses mkcert, not LocalMachine\Root writes.** New `Patching/CertificationUtility.cs` invokes `mkcert -install` (idempotent) and `mkcert -cert-file ... -key-file ... localhost`. The launcher hard-errors with install instructions if mkcert is missing rather than silently degrading.
- **authlogin.dll patching gated on FileVersion == 3.3.0.6.** `AuthLoginPatcher.VerifyVersion()` runs before Read and Write; mismatched builds are refused with a clear error rather than corrupting unrelated bytes via stale offsets.
- **One-shot backup rule for in-place client patches.** `Launcher.EnsureBackup(src, backup)` copies `src → backup` exactly once (when the backup doesn't yet exist) and never modifies the backup again. Operators wanting to re-patch from a clean baseline copy the `.orig` back themselves.
- **Hostnames in source comments / Host: header literals scrubbed.** `proxy/SSL_Connection.cpp`, `login-server/Net7SSL/SSL_Connection.cpp`, `login-server/Net7SSL/Net7SSL.cpp`, `proxy/Net7.cpp`, `deploy/Net7Config.cfg` — every `local.net-7.org` literal swapped for `localhost`.
- **Net-7 client-side INIs backed up + scrubbed.** `client/mods/Data/client/ini/auth.ini` and `rg_regdata.ini` now point at `localhost`; `.orig` siblings preserve the originals. The strings in localised resource files (`RG_C2.INI`, `RG_X3.INI`) are user-visible messaging copy and were left alone — they're not connection endpoints.
- **OpenSSL 3 confirmed end-to-end on Linux.** `ldd` on the freshly-built `net7proxy` and `net7ssl` shows `libssl.so.3` + `libcrypto.so.3` (OpenSSL 3.0.13). Docs page covers cipher suite negotiation and the `TLS_server_method()` vs legacy `SSLv23_server_method()` alias situation.
- **Deferred deliberately**: auto-update (operator updates binaries out of band; launcher's update machinery stays dormant), packet-opt / reorder CLI flags (not reintroduced), outbound TLS wiring for `RegisterSectorServer` (Linux path is still short-circuited; plumbing for `g_UpstreamHost` exists for when that lands), dev-tool cleanup (`tools/sector-editor`, `tools/mob-editor`, `tools/enb-ini-parser`, `tools/toolslauncher`, `tools/enbpatcher-avalonia`, `tools/toolspatcher-avalonia` still reference the old DB/patch hosts — separate task, not on the client traffic path).

## 2026-05-28 — launchnet7-avalonia: in-process plaintext→TLS auth relay

- **The launcher owns the loopback hop, not the WINE prefix.** authlogin.dll's WINE-side TLS path against the dev self-signed cert was flaky (`SSL_accept failed (err=1)` on Net7SSL most of the time, occasional success — schannel/WINE-11.8 interaction with self-signed Root\Certificates entries). Rather than chase WINE schannel, the launcher now runs an in-process `TcpListener`+`SslStream` bridge bound to `IPAddress.Loopback`. authlogin.dll talks plaintext HTTP to that listener; the listener wraps each connection as TLS to the actual upstream. WINE schannel is removed from the auth path entirely.
- **Loopback target is HARDCODED — no knob.** `LocalAuthRelay.ListenPort` is `const`; `Launcher.PatchAuthLoginFile` writes `Port=ListenPort, UseHttps=false` unconditionally (no setting feeds either value); `Launcher.PatchRegistry` hardcodes `HKLM\Software\EACom\AuthAuth\AuthLoginServer=localhost` on the WINE branch; `WindowsRegistryHelpers.EnsureRegistered` does the same on Windows. There is no user-facing way to point authlogin.dll at a remote host. Reason: even a single misconfiguration would let plaintext bytes leave the box, which is exactly what the relay exists to prevent.
- **Dead knobs purged.** Same change deletes `LaunchSetting.UseSecureAuthentication` / `UseClientDetours` / `UseLocalCert`, `HostConfig.SupportsSecureAuthentication` / `SecureAuthenticationPort` (collapsed into the single TLS upstream `AuthenticationPort`), `UserSettings.UseSecureAuthentication` / `UseClientDetours` / `UseLocalCert`, the `supportSecureAuthentication`/`authenticationPort`-plain attributes in `LauncherConfig.Load`, the Detours branch in `LaunchClient` + `/LC` branch in `LaunchNet7Proxy`, the `UseLocalCert` hostname swap in `PatchNetworkIniFile`, the `UseSecureAuthentication`-driven scheme picker in `PatchAuthIniFile` and `PatchRegDataFile` (both hardcoded `https`), and the WinForms-era "Authentication Port (HTTPS):" label (now "Upstream Auth Port (TLS):"). Reason: dead-code rule — once `Launcher.PatchAuthLoginFile` and the upstream-only TLS design made these knobs structurally unreachable, the consts/branches/properties were just confusing noise.
- **Verify policy splits by upstream, syntactically.** `LocalAuthRelay.IsLoopback(host)` accepts `localhost` (case-insensitive) or `IPAddress.IsLoopback(parsed-IP)` — **no DNS lookup**. Loopback upstream → `userCertificateValidationCallback` returns `true` (dev self-signed cert on the same box). Non-loopback upstream → null callback (full OS-trust-store validation). The syntactic check matters: if we resolved the host first, a poisoned DNS answer claiming `prod.example.com→127.0.0.1` would downgrade verify; with a syntactic check, only literal `localhost`/`127.0.0.0/8`/`::1` qualify.
- **Plaintext on the wire is acceptable only on loopback.** `TcpListener` binds `IPAddress.Loopback` specifically (not `IPAddress.Any`). Plaintext bytes never traverse the host's network interfaces; an off-host attacker would need to already own the loopback interface (i.e. the box) to MITM the plaintext hop. This is the same threat model as Postgres' default `127.0.0.1` listener.
- **Cert trust no longer needed in the WINE prefix.** `just play-local` drops the `just trust-cert` call. The dev cert lives only on the host's filesystem; the launcher's `SslStream` upstream-verify=skip on loopback means no party needs to trust it. (Remote deploys still need a real CA-signed cert on the upstream — that's the only path where validation is enforced.)
- **Launcher lifetime is tied to the game session.** `MainWindow.OnPlayClick` no longer closes the window after Launch — it disables Play and shows "Client running. Hit Quit when you're done to tear everything down." `OnClosing` calls `_activeLauncher?.AuthRelay?.Dispose()` so the listener doesn't outlive the GUI. The relay, the wine proxy, and the client all live as long as the launcher window does.
- **Server posture untouched.** Per the CLAUDE.md server-integrity rule, Net7SSL was NOT loosened to accept plaintext. The relay is a client-side adapter that speaks TLS to the server exactly as a stock authlogin.dll would; the server still requires TLS on 443 (mapped to host 4443 in the dev stack).


## 2026-05-29 -- Phase X: MD5 -> Argon2id password hash modernization

- **Argon2id over bcrypt.** Both are credible modern choices; Argon2id wins on memory-hardness (configurable MEMLIMIT) which makes GPU/ASIC attacks meaningfully more expensive than bcrypt's CPU-bound cost. libsodium's `crypto_pwhash_str` + `crypto_pwhash_str_verify` API is also markedly simpler than wiring bcrypt (no per-call cost factor management, no separate salt buffer, no hash-output-length parameter), and libsodium is already widely vendored. PHC self-describes (algorithm + params + salt + hash), so future params changes don't need a schema migration. Calibrated to the INTERACTIVE profile (m=64MiB, t=2, p=1) -- ~70ms per call, matches the wire-facing `/AuthLogin` latency budget.
- **Destructive migration, no back-compat.** Per user directive ("no back compat needed"), the schema renames `accounts.password varchar(40)` to `accounts.password_phc text` and drops all existing MD5 hashes. Existing accounts re-provision via `ChangePassword` or get re-seeded from `seed.sql`. The dev admin row carries an Argon2id PHC for plaintext `'devadmin'`; rotate before any non-dev use.
- **Hashing happens in-process, never via SQL.** Pre-Phase-X, both the C++ `AccountManager::ValidateAccount` (server) and `LinuxAuth::ValidateAccountLinux` (login-server) compared `UPPER(MD5(?))` server-side -- the plaintext crossed the Postgres wire and showed up in pg_stat_statements unless explicitly redacted. Post-Phase-X, both paths `SELECT password_phc FROM accounts WHERE username = $1` and run `crypto_pwhash_str_verify` in-process: plaintext never leaves the host process address space. Parameterised `$1` everywhere -- no concatenation, no format strings.
- **Test fixtures use a constant PHC, not a runtime hasher.** `TestAccounts.cs` previously computed `UPPER(encode(digest(@password, 'md5'), 'hex'))` via pgcrypto. Post-Phase-X it inserts a precomputed Argon2id PHC string constant for the shared password "testpw" and trusts the server's `crypto_pwhash_str_verify` to match it. Avoids pulling a C# Argon2 implementation into the test suite. pgcrypto extension dependency dropped from `ServerFixture`.
- **IDENTITY sequence reset added to `convert.sh`.** Side effect of doing a clean rebuild: `seed.sql` inserts explicit `accounts.id = 1` (admin) but Postgres's `GENERATED BY DEFAULT AS IDENTITY` does not advance the sequence on explicit-value inserts. First `INSERT ... RETURNING id` from `TestAccounts.New` then got nextval=1 and PK-collided with admin. Convert.sh now appends a `DO $reset_identity$` block that walks `information_schema.columns WHERE is_identity = 'YES'` and `setval`s each sequence past `MAX(id)`. Pre-existing latent bug; Phase X clean-rebuild was what exposed it.
- **Legacy MFC `login-server/Net7SSL/AccountManager.cpp` deleted.** 1028 lines of Win32-only MFC code wrapped entirely in `#ifdef WIN32`, replaced by `LinuxAuth.cpp` since Phase J. Held the last `WHERE password = MD5(...)` and `password = MD5(...)` string-concat patterns in the source tree. Per the no-dead-code rule and Phase X W5 scope ("Delete or rewrite"), deleted outright. `login-server/Net7Mysql/Tab2.cpp` MFC dialogs are out of scope (Phase Q deletion radar).
- **libsodium vendored via Debian apt.** Build-stage apt-get pulls `libsodium-dev`; runtime stage pulls `libsodium23`. Same pattern as libpqxx. `pkg_check_modules(LIBSODIUM REQUIRED IMPORTED_TARGET libsodium)` in both `server/CMakeLists.txt` and `login-server/Net7SSL/CMakeLists.txt`.
...
**Why.** The runtime Postgres tables for these five categories are already densely and authoritatively populated:

| category | runtime table | rows | what the runtime has that the wiki lacks |
|---|---|---|---|
| sector navs | `sector_objects` | 9382 | real `base_asset_id`, runtime coords |
| mobs | `mob_base` | 2042 | integer asset id, faction id, AI fields |
| missions | `missions` | 364 | `mission_XML` on every row (the runnable mission) |
| item drops | `mob_items` | 8583 | authoritative loot rows w/ real ids |
| prospecting | `sector_objects_harvestable` | 882 | runtime harvestable fields |

The wiki JSONL is a **name-level near-duplicate in an incompatible form**: nav coordinates are the runtime coordinates divided by ~1000 (proof: wiki "Sado Pit" (-152.74,-63.89) == runtime "Sado Pit" (-152740,-63860) in sector 1076), it carries no asset ids, no faction ids, no mission XML, and drop rates are mostly literal "?". Coverage overlap (wiki names already present in runtime): sectors 85%, mobs 77%, item-drop mob refs 77%, missions 25%. Injecting it would produce wrong-position, asset-less duplicate objects and non-runnable missions.

**Authority.** CLAUDE.md server-integrity rule: never make the server diverge from the real EnB server; the only escape hatch is a PRIMARY source (retail packet capture, decompilation, or first-hand Net-7 doc). A wiki scrape is explicitly **not** a primary source, and Phase Y's own process gate says so. So the runtime stays; the import is what adapts (i.e. doesn't happen).

**Evidence artifact.** `tools/dataimport-jsonl/analyze_wiki_coverage.py` -- read-only, reproducible; SELECTs the runtime DB, reads the JSONL, prints the coverage table above. Run: `python3 tools/dataimport-jsonl/analyze_wiki_coverage.py`.
