...
# changed. Idempotent.
#
# -Tag selects which version SUFFIX to run across the services -- 'latest'
# (default, from IMAGE_TAG in .env) or a pinned 'vN'. The compose file resolves
# it as enb:server-<tag> / enb:net7go-<tag> / enb:freya-online-<tag>. (The proxy
# is client-side and never deployed here.)
param([string]$Tag)
