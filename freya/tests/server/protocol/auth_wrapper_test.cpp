...
std::vector<unsigned char> MakeRecord(uint8_t version, const unsigned char token[16],
                                      int32_t player_id, size_t payload_len = 8) {
...
TEST(AuthWrapper, CorrectToken_GameplayDatagram_IsAccepted) {
    ...
    FixedTokenLookup lookup;
    FillToken(lookup.token, 0xA0); // matches
    const unsigned char* inner = nullptr;
    int inner_len = 0;

    auto v = net7::CheckAuthWrapper(rec.data(), rec.size(), kMaxBuffer, lookup, &inner, &inner_len);
...
TEST(AuthWrapper, WrongToken_WellFormedOtherwise_IsDroppedAsTokenMismatch) {
    ...
    FillToken(forged, 0xB0);
    auto rec = MakeRecord(net7::kAuthWrapperVersion, forged, /*player_id=*/0x1234);

    FixedTokenLookup lookup;
    FillToken(lookup.token, 0xA0); // the REAL token
    const unsigned char* inner = (const unsigned char*)0xdead;
    int inner_len = -1;
...
TEST(AuthWrapper, PreAuthDatagram_PlayerIdZero_SkipsTokenCheck) {
    ...
    auto v = net7::CheckAuthWrapper(rec.data(), rec.size(), kMaxBuffer, must_not_be_called, &inner,
                                    &inner_len);
    EXPECT_EQ(v, net7::AuthWrapperVerdict::Accept);
}
