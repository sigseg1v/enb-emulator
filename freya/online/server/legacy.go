...
// Freya Online -- byte-exact reimplementation of the legacy Net7SSL game-auth
// endpoints. These responses go on the wire to the real client.exe / launcher /
// sector server, so every byte MUST match the C++ Net7SSL (login-server/Net7SSL/
...
// connection hijacked so net/http injects nothing of its own.
//
// Primary source for the exact bytes: login-server/Net7SSL/LinuxAuth.cpp
// (HttpResult / MakeNotFound / HandleAuthLogin / HandleTouchSession /
// HandleSectorServer / HandleCertificate / HandleUpdateCheck) and the tag/
// version macros in SSL_Connection.h + Net7SSL.h.
...
// handleUpdateCheck -- HandleUpdateCheck. With no manifest source configured the
// C++ returns 503 on a cold cache (fail-closed); we mirror that. A populated
// manifest path is out of scope for the Go port until the patcher source moves.
func (l *legacyServer) handleUpdateCheck(r *http.Request) []byte {
	if l.cfg.PatcherManifestURL == "" {
		return legacyServiceUnavailable
	}
	// Manifest-backed update decisions are not yet ported; fail closed rather
	// than emit a wrong UP_TO_DATE/UPDATE_NEEDED.
	_ = r
	return legacyServiceUnavailable
}
