...
function VaultGrid({
  title, avatar, avatars, onPick, slots, selSlot, onSelectAvatar, exclude,
}: {
  title: string;
  avatar: string;
  avatars: string[];
      ...
      <div className="vault__head">
        ...
        <div className="vsel">
          <select value={avatar} onChange={e => onSelectAvatar(e.target.value)}>
            {avatars.map(c => (
              <option key={c} value={c} disabled={c === exclude}>{c}</option>
            ))}
          </select>
        </div>
      </div>
      <div className="vault__sub" style={{ padding: '0 12px 6px', fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-faint)' }}>
...
export function Vault({
  vaultStorage, avatars, reload, toast,
}: {
  ...
  useEffect(() => { setSel(null); }, [left, right]);
      ...
      <div className="page__head">
        <div>
          ...
          <div className="page__sub">
            Move items between two of your characters &middot; {VAULT_SLOT_COUNT} slots per vault &middot; both must be offline
          </div>
        </div>
      </div>
              ...
              {armed && (
                <div className="vxfer__preview" style={{ marginTop: 12, textAlign: 'center' }}>
                  ...
                  <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-dim)' }}>
                    ...
                    {armed.quality != null && <> &middot; <Quality value={armed.quality} /></>}
                  </div>
                </div>
              )}
            </div>

            <VaultGrid title="Source / Destination" avatar={right} avatars={avatars}
              slots={rightSlots} selSlot={sel?.side === 'right' ? sel.slot : null}
              onPick={slot => setSel({ side: 'right', slot })}
              onSelectAvatar={setRight} exclude={left} />
          </div>
        </div>
      )}
    </div>
