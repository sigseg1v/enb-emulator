        ...
        <div className={styles.ahrow + (active ? ' ' + styles.ahrowActive : '')} onClick={onClick}>
            ...
            <div style={{ minWidth: 0, '--rcolor': r.color } as Vars}>
                <div className={styles.ahrowName} style={{ color: r.color }}>
                    {listing.item.name}
                    {listing.stack > 1 ? ` ×${listing.stack}` : ''}
                </div>
                <div className={styles.ahrowMeta}>
                    L{listing.item.level} &middot; {listing.item.slot}
                </div>
            </div>
            <div
                className={
                    styles.ahrowSeller +
                    (listing.seller === 'AhBot' ? ' ' + styles.ahrowSellerBot : '')
                }
            >
                {listing.seller}
            </div>
            <div className={styles.ahrowNum}>
                {listing.quality != null ? (
                    <Quality value={listing.quality} />
                ) : (
                    <span className="dash">-</span>
                )}
            </div>
            <div className={styles.ahrowNum}>
                <TimeBand band={listing.band} showLabel={false} />
            </div>
            <div className={styles.ahrowPrice}>
                <b>
                    <Credits value={listing.bid} />
                </b>
            </div>
            <div className={`${styles.ahrowPrice} ${styles.ahrowPriceBuyout}`}>
                {listing.buyout != null ? (
                    <b>
                        <Credits value={listing.buyout} />
                    </b>
                ) : (
                    <span className="dash">-</span>
                )}
    ...
    const shown = useMemo(
        () =>
            listings.filter(l => {
                ...
                if (
                    q &&
                    !l.item.name.toLowerCase().includes(q.toLowerCase()) &&
    ...
    return (
        <div className={styles.ah}>
                    ...
                    <div className={styles.chips}>
                        {AH_FILTERS.map(f => (
                            <button
                                key={f.key}
                                className={
                                    styles.chip + (filter === f.key ? ' ' + styles.chipOn : '')
                                }
                    ...
                    {shown.length === 0 ? (
                        <div className="empty" style={{ padding: '60px 0' }}>
                            <div className="empty__inner">
                                <div className="empty__glyph">⌕</div>No listings match
                            </div>
                        </div>
                    ) : (
                        shown.map(l => (
            ...
            <div className={styles.ahDetail}>
                {!active ? (
                    <div className="hud-panel empty" style={{ height: '100%' }}>
                        <div className="empty__inner">
                            <div className="empty__glyph">◈</div>Select a listing
                        </div>
                    </div>
                ) : (
                    <>
                        ...
                        <div className={`${styles.bidbox} hud-panel hud-corners`}>
                            <div className={styles.bidboxRow}>
                                ...
                                <span
                                    className={
                                        active.seller === 'AhBot' ? styles.ahrowSellerBot : ''
                                    }
                            ...
                            <div className={styles.bidboxRow}>
                                ...
                                <span
                                    className={
                                        'mono ' +
                                        (active.highBidder ? styles.bidder : styles.bidderNone)
                                    }
                                >
                                    {active.highBidder || 'No bids yet'}
                                </span>
                            </div>
                            <div className={styles.bidboxRow}>
                                <span>Current bid</span>
                                <span className={styles.bidboxBig}>
                                    <Credits value={active.bid} />
                                </span>
                            </div>
                            {active.buyout != null && (
                                <div className={styles.bidboxRow}>
                                    <span>Buyout</span>
                                    <span
                                        className={styles.bidboxBig}
                                        style={{ color: 'var(--warm)' }}
                                    >
                                        <Credits value={active.buyout} />
                                    </span>
                                </div>
                            )}
                            <p className={styles.bidboxHint}>
                                Next bid must be at least{' '}
                                <b style={{ color: 'var(--text-dim)' }}>+{fmtNum(minInc)} cr</b> (1%
                                of item value). Quantity in stack: {active.stack}.
                            </p>
                            <div className={styles.bidboxActions}>
                                <button className="btn" onClick={placeBid} disabled={busy}>
                                    Bid {fmtNum(active.bid + minInc)}
                                </button>
                                <button
                                    className="btn btn--warm"
                                    onClick={buyoutNow}
                                    disabled={busy || active.buyout == null}
                                >
                                    Buyout
                                </button>
                            </div>
        ...
        <div className={styles.ahrow} style={{ cursor: 'default' }}>
            ...
            <div className={styles.ahrowPrice}>
                {listing.currentBid != null ? (
                    <b>
                        <Credits value={listing.currentBid} />
                    </b>
                ) : (
                    <span className="dash">-</span>
                )}
        ...
        <div className={`${styles.pform} hud-panel hud-corners`}>
            ...
            >
                ...
                <div>
                    ...
                    <div className={styles.pformPsub}>
                        L{item.level} &middot; {item.slot}
                        {slotData.quality != null && (
                            <>
                                {' '}
                                &middot; Quality <Quality value={slotData.quality} />
                            </>
                        )}
                    </div>
                </div>
                ...
                <div className={styles.seg}>
                    {DURATIONS.map(d => (
                        <button
                            key={d.key}
                            className={
                                styles.segBtn + (duration === d.key ? ' ' + styles.segBtnOn : '')
                            }
            ...
            <div className={styles.pformField}>
                ...
                <div className={styles.stepper}>
                    ...
                    <button
                        ...
                        onClick={() => setQty(v => Math.min(maxStack, v + 1))}
                    >
                        +
                    </button>
                </div>
                <div className={styles.stepperMax}>
                    {maxStack === 1
                        ? 'Single item -- fixed at 1'
                        : `Up to ${maxStack} available in this slot`}
                </div>
            </div>
            ...
            <div className={styles.pformField}>
                <span className={styles.pformLbl}>
                    Buyout price{' '}
            ...
            <p className={styles.pformFee}>
                Auction House fee <b>10%</b> &middot; <b>{fmtNum(fee)} cr</b> collected now. Unsold
                items return 95% to your mailbox.
            </p>
            <button
                className={`btn btn--primary ${styles.pformPost}`}
                onClick={post}
                disabled={busy}
            >
...
function SellView({
    ...
    async function post(p: PostListingInput) {
        ...
        try {
            ...
            toast(
                `Listed ${src.item.name}${p.stack > 1 ? ` ×${p.stack}` : ''} -- ${fmtNum(p.fee)} cr fee collected`,
            );
            setSelSlot(null);
            await reload();
        } catch (err) {
        ...
        <div className={styles.sell}>
            <div className={styles.sellLeft}>
                            ...
                            slots.map(s => {
                                ...
                                return (
                                    <div
                                        ...
                                        className={
                                            'vslot' +
                                            (selSlot === s.slot ? ' vslot--sel' : '') +
                                            (untradable ? ' vslot--locked' : '')
                                        }
                                        style={{ '--rcolor': r.color, '--rglow': r.glow } as Vars}
                                        title={
                                            untradable
                                        ...
                                        onClick={() =>
                                            untradable
                                                ? toast(
                                                      'That item is flagged Not Tradable and cannot be listed.',
                                                  )
                                                : setSelSlot(s.slot)
                                        }
                                    >
                                        ...
                                        {untradable && (
                                            <span className="vslot__lock" title="Not Tradable">
                                                ⊘
                                            </span>
                                        )}
                                    </div>
                ...
                {slotData ? (
                    ...
                    <div className={`hud-panel ${styles.sellHint}`}>
                        <div>
                            <div className="empty__glyph">▲</div>Select a vault item
                            <br />
                            to set your listing terms
                        </div>
                    </div>
                )}
            </div>
            ...
            <div className={`${styles.sellMain} hud-panel`}>
                <div className={styles.sellMainhead}>
                    ...
                    <span>
                        {slotData ? 'Set terms, then list →' : 'Select a vault item to list ◂'}
                    </span>
                </div>
                <div className={styles.ahtable}>
                    <div className={`${styles.ahrow} ${styles.ahrowHead}`}>
                    ...
                    {mine.length === 0 ? (
                        <div className="empty" style={{ padding: '60px 0' }}>
                            <div className="empty__inner">
                                <div className="empty__glyph">◈</div>No active listings
                            </div>
                        </div>
                    ) : (
        ...
        <div className="page">
            <div className="page__head">
                ...
                <div className="subtabs">
                    <button
                        className={'subtab' + (sub === 'buy' ? ' subtab--active' : '')}
                        onClick={() => setSub('buy')}
                    >
                        <span className="subtab__glyph">⌕</span> Buy
                    </button>
                    <button
                        className={'subtab' + (sub === 'sell' ? ' subtab--active' : '')}
                        onClick={() => setSub('sell')}
                    >
