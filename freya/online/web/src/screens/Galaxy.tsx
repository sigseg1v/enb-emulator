...
const STAR_PATH =
    'M0,-13 L3.2,-4.5 L12.4,-4 L5.2,1.7 L7.6,10.5 L0,5.5 ' +
...
const RAW: [string, number, number][] = [
    ['Antares Frontier', 187, 282],
    ['Dahin', 414, 178],
    ['Dahin Planet', 435, 115],
    ['Kailaasa', 496, 199],
    ["Kitara's Veil", 336, 147],
    ["Vishao's Cove", 336, 251],
    ['Yokan', 435, 283],
    ['Ishuan', 632, 360],
    ['Menorb', 569, 424],
    ['Swooping Eagle', 313, 530],
    ['Swooping Eagle Planet', 385, 464],
    ['Xipe Totec', 413, 588],
    ['Mazzaroth (old)', 159, 475],
    ['FishBowl', 76, 640],
    ['Carpenter', 356, 795],
    ['Cooper', 127, 927],
    ['Glenn', 242, 729],
    ["Glory's Orbit", 242, 861],
    ['Grissom', 242, 993],
    ['Grissom Planet', 242, 927],
    ['Shepard', 356, 927],
    ['Slayton', 174, 805],
    ['AstroBelt Alpha', 916, 587],
    ['AstroBelt Beta', 964, 464],
    ['AstroBelt Gamma', 930, 382],
    ['Earth', 960, 654],
    ['Equatorial Earth', 1038, 694],
    ['Europa', 766, 502],
    ['Ganymede', 787, 409],
    ['High Earth', 860, 683],
    ['Io', 844, 333],
    ...
    ['Mars Gamma', 1156, 360],
    ['Mercury', 1163, 502],
    ['Pluto & Charon', 1127, 567],
    ['Saturn', 878, 522],
    ['Uranus', 1025, 556],
    ['Venus', 1089, 417],
    ['Ceres / Thule', 1196, 435],
    ['Neptune', 1053, 497],
    ['Akerons Gate', 1100, 638],
    ['Mondara (old)', 1097, 144],
    ['Arduinne', 707, 793],
    ['Arduinne Planet', 713, 718],
    ['Inverness', 611, 827],
    ['Inverness Planet', 567, 751],
    ['New Edinburgh', 618, 671],
    ['Aganju', 724, 1014],
    ['Moto', 807, 984],
    ['der Todesengel (old)', 1058, 981],
    ['Witberg', 1134, 927],
    ['Zweihander', 1080, 851],
    ['Zweihander Planet', 1007, 894],
    ["Blackbeard's Wake", 479, 1159],
    ['Paramis', 556, 1115],
    ['Altair III', 1428, 436],
    ['Nostrand Vor', 1472, 511],
    ['Nostrand Vor Planet', 1385, 553],
    ['Adriel Prime', 1362, 814],
    ['Margesi', 1273, 798],
    ['Ardus (old)', 1290, 1082],
    ['Roc', 1649, 613],
    ['Endriago', 1644, 239],
    ['Endriago Planet', 1742, 189],
    ['Lagarto', 1731, 342],
    ['Lagarto Moon Risco', 1796, 265],
    ['Primus', 1407, 193],
    ['Primus Planet', 1337, 127],
    ['Tarsis', 1294, 214],
    ['Aragoth Prime', 1695, 1080],
    ['Fenris', 1524, 998],
    ['Freya', 1577, 799],
    ['Jotunheim', 1722, 800],
    ['Muspelheim', 1775, 1144],
    ['Nifleheim Cloud', 1523, 883],
    ['Odin Rex', 1803, 883],
    ["Odin's Belt", 1875, 971],
    ['Ragnarok', 1639, 868],
    ['Valkyrie Twins', 1676, 971],
    ["Varen's Girdle", 1577, 1090],
];
...
const LANES: Lane[] = (() => {
    ...
    GATES.forEach(([a, b]) => {
        ...
        const px = -dy / len,
            py = dx / len; // unit perpendicular to the lane
...
const { STARS, NEBULAE } = (() => {
    ...
    const nebs: Neb[] = SYSTEMS.map((sm, ri) => {
        ...
        const fs = Math.max(17, Math.min(78, sm.r * 0.3));
        const big = sm.r >= 150;
...
export function Galaxy() {
    ...
    useEffect(() => {
        ...
        api.fetchGalaxy()
            ...
            .catch(e => {
                if (alive) setErr(e instanceof Error ? e.message : 'failed to load galaxy');
            });
        return () => {
            alive = false;
        };
    ...
    useEffect(() => {
        ...
        const tick = () =>
            api
                .fetchGalaxyOccupancy()
                .then(o => {
                    if (alive) setOcc(o);
                })
                .catch(() => {
                    /* keep last good occupancy */
                });
        tick();
    ...
    useEffect(() => {
        ...
        const tick = () =>
            api
                .fetchMyAvatars()
                .then(a => {
                    if (alive) setMyAvatars(a);
                })
                .catch(() => {
                    /* keep last good locations */
                });
        tick();
        const h = setInterval(tick, OCCUPANCY_POLL_MS);
        return () => {
    ...
    const flowDots = useMemo(() => {
        ...
        for (const L of LANES) {
            const aId = idByNorm[gnorm(L.a)],
                bId = idByNorm[gnorm(L.b)];
            if (!aId || !bId) continue;
            ...
            <div className="page__head">
                <div>
                    ...
                    <div className="page__sub">
                        Live sector chart &middot; <b>{total}</b> pilot{total === 1 ? '' : 's'}{' '}
                        online &middot; {NODES.length} charted sectors
                    </div>
                </div>
                <div className={styles.gxlegend + (hl ? ' ' + styles.dim : '')}>
                    {FACTION_LEGEND.map(f => (
                        <span
                            ...
                            className={
                                styles.gxlegendItem +
                                (hl?.kind === 'faction' && hl.f === f ? ' ' + styles.on : '')
                            }
                            onMouseEnter={() => setHl({ kind: 'faction', f })}
                            onMouseLeave={() =>
                                setHl(h => (h?.kind === 'faction' && h.f === f ? null : h))
                            }
                        >
                    ...
                    <span
                        className={
                            styles.gxlegendOnline + (hl?.kind === 'online' ? ' ' + styles.on : '')
                        }
            ...
            <div className={styles.mapwrap} ref={wrapRef} onMouseMove={onMove}>
                    ...
                    <defs>
                        ...
                        {Object.keys(F).map(f => (
                            ...
                            >
                                ...
                                <stop offset="45%" stopColor={col(f, 0.42, 0.3)} />
                                <stop offset="100%" stopColor={col(f, 0.3, 0.0)} />
                            </radialGradient>
                        ))}
                        {Object.keys(F).map(f => (
                            <radialGradient
                            ...
                            >
                                ...
                                <stop offset="100%" stopColor={col(f, 0.5)} />
                            </radialGradient>
                        ))}
                    </defs>
                            ...
                            <g key={nb.sm.name}>
                                <g
                                    ...
                                    style={
                                        {
                                            '--nd': nb.nd + 's',
                                            '--dx': nb.dx.toFixed(1) + 'px',
                                            '--dy': nb.dy.toFixed(1) + 'px',
                                        } as Vars
                                    }
                                >
                                    <ellipse
                    ...
                    <g>
                        {GRID.v.map(gx => (
                            <line
                                ...
                                x2={gx}
                                y2={H}
                                stroke="oklch(0.5 0.04 250 / 0.05)"
                                strokeWidth={1}
                            />
                        ))}
                        {GRID.h.map(gy => (
              ...
              equidistant (staggered start phase) along the same path. */}
                    ...
                    <g>
                        {NODES.map(s => {
                            ...
                            const labelDy =
                                s.n === 'Gallina' ? (s.hub ? 30 : 21) : s.n === 'Sirius' ? -8 : 0;
                                ...
                                >
                                    ...
                                    {s.hub && (
                                        <circle
                                            ...
                                            opacity={0.5}
                                            style={{
                                                animationDelay: ((s.x % 30) / 10).toFixed(1) + 's',
                                            }}
                                        />
                                    )}
                      ...
                      the waves' own fade animation -- robust, no var() in keyframes. */}
                                    {online > 0 &&
                                        (() => {
                                            const intensity =
                                                maxOnline > 0
                                                    ? Math.max(0.35, online / maxOnline)
                                                    : 0;
                                            return (
                                                <>
              ...
              above the sector nodes. */}
                ...
                <div className={styles.readout}>
                    <div>
                        SECTOR GRID &middot; <b>FREYA-9</b>
                    </div>
                    <div className={styles.readoutCrosshair} ref={coordsRef}>
                        [ 0000 &middot; 0000 ]
                    </div>
                </div>
                        ...
                        <div className={styles.tipIn}>
                            ...
                            <div className={styles.tipRows}>
                                ...
                                <div className={styles.tipRow}>
                                    <span>Coord</span>
                                    <span>
                                        {pad4(hovered.x)} &middot; {pad4(hovered.y)}
                                    </span>
                                </div>
                            </div>
                        </div>
