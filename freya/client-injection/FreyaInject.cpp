...
static int InjectOne(HANDLE hProcess, const char *dllPath)
{
    ...
    void *remote = VirtualAllocEx(hProcess, NULL, n,
    ...
    { fprintf(stderr, "FreyaInject: WriteProcessMemory failed (%lu)\n", GetLastError()); VirtualFreeEx(hProcess, remote, 0, MEM_RELEASE); return 5; }
...
int main(int argc, char **argv)
{
    if (argc < 3)
    {
        fprintf(stderr, "FreyaInject: usage: FreyaInject.exe <target-exe> <dll1> [dll2 ...] -- [args...]\n");
        return 2;
    }
    ...
    for (int i = 2; i < argc; ++i)
        if (strcmp(argv[i], "--") == 0) { delim = i; break; }

    if (delim < 0)
    {
        dlls.push_back(argv[2]);
        for (int i = 3; i < argc; ++i) clientArgs.push_back(argv[i]);
    }
    else
    {
        for (int i = 2; i < delim; ++i)     dlls.push_back(argv[i]);
        for (int i = delim + 1; i < argc; ++i) clientArgs.push_back(argv[i]);
    }
