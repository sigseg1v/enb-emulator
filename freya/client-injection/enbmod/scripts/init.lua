...
-- ---------------------------------------------------------------------------
-- HIDE THE NATIVE WIDGETS WE REPLACE (ON by default, owner-directed 2026-06-13).
--
...
--
-- CAVEAT: this is owner-directed ON ahead of real-client confirmation. The paint
-- addresses are pinned by behavioural analysis, but "an early ret hides the
-- widget without side effects" is only provable on the real client. If the client
-- crashes on load, comment these two lines out (CV-AS-HIDE-VITALS / -XP in
-- plans/29-client-verification.md track the confirmation).
