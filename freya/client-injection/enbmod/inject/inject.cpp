...
int main(int argc, char** argv) {
    ...
    if (!th) { fprintf(stderr, "[inject] CreateRemoteThread failed: %lu\n", GetLastError());
               CloseHandle(h); return 7; }

    WaitForSingleObject(th, 10000);
    DWORD ret = 0; GetExitCodeThread(th, &ret);
    printf("[inject] remote LoadLibraryA returned 0x%lx %s\n",
           ret, ret ? "(loaded)" : "(FAILED -- module handle 0)");
