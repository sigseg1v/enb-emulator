...
void on_tick() {
    ...
    if (++g_tick_count >= 120) {
        g_tick_count = 0;
        FILETIME ft;
        if (file_mtime(g_init_path, ft) && CompareFileTime(&ft, &g_init_mtime) != 0) {
...
DWORD WINAPI worker(LPVOID) {
    ...
    // lib/modloader, lib/json, ...). The mod loader appends each mod's own dir.
    {
        ...
        std::string lp = "enb.script_dir = [[" + scripts +
                         "]]\n"
                         "package.path = [[" +
                         scripts +
                         "\\?.lua]] .. ';' .. "
        ...
        if (luaL_dostring(g_L, lp.c_str()) != LUA_OK)
            lua_pop(g_L, 1);
    }
    ...
    if (!enb::hooks::mh_init())
        enb::logf("hooks::mh_init failed -- game hooks unavailable");
