...
static BOOL CALLBACK enum_cb(HWND h, LPARAM lp) {
    ...
    if (pid == c->pid && IsWindowVisible(h) && GetWindow(h, GW_OWNER) == nullptr) {
        c->hwnd = h; return FALSE;   // stop at first visible owned-by-none top-level
    }
    return TRUE;
}
void* game_hwnd() {
...
static LPARAM key_lparam(int vk, bool down) {
    ...
    LPARAM lp = (1) | (scan << 16);
    if (!down) lp |= (1u << 30) | (1u << 31);  // previous-state + transition (key-up)
    return lp;
}

bool post_key(int vk, bool down) {
    HWND h = (HWND)game_hwnd();
    if (!h) { logf("actions: no game window"); return false; }
    return PostMessageA(h, down ? WM_KEYDOWN : WM_KEYUP, (WPARAM)vk, key_lparam(vk, down)) != 0;
}
bool tap_key(int vk) { bool a = post_key(vk, true); bool b = post_key(vk, false); return a && b; }
