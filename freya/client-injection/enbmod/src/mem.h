...
inline bool readable(const void* p, size_t len) {
    ...
    while (a < end) {
        ...
        bool ok = prot == PAGE_READONLY || prot == PAGE_READWRITE || prot == PAGE_EXECUTE_READ ||
                  prot == PAGE_EXECUTE_READWRITE || prot == PAGE_WRITECOPY ||
                  prot == PAGE_EXECUTE_WRITECOPY;
        if (!ok || (mbi.Protect & PAGE_GUARD))
            return false;
...
inline bool writable(const void* p, size_t len) {
    ...
    if (mbi.State != MEM_COMMIT)
        return false;
    DWORD prot = mbi.Protect & 0xFF;
    return prot == PAGE_READWRITE || prot == PAGE_EXECUTE_READWRITE || prot == PAGE_WRITECOPY ||
           prot == PAGE_EXECUTE_WRITECOPY;
}
...
template <typename T> inline T read(uintptr_t addr, T def = T{}) {
    if (!readable((const void*)addr, sizeof(T)))
        return def;
    if (__builtin_setjmp(g_guard_jmp)) {
        g_guard_on = 0;
        return def;
    }
    g_guard_on = 1;
...
inline uint8_t u8(uintptr_t a) {
    return read<uint8_t>(a);
}
inline uint16_t u16(uintptr_t a) {
    return read<uint16_t>(a);
}
inline uint32_t u32(uintptr_t a) {
    return read<uint32_t>(a);
}
inline int32_t i32(uintptr_t a) {
    return read<int32_t>(a);
}
inline float f32(uintptr_t a) {
    return read<float>(a);
}
inline double f64(uintptr_t a) {
    return read<double>(a);
}
inline uintptr_t ptr(uintptr_t a) {
    return read<uintptr_t>(a);
}
...
inline size_t readable_run(uintptr_t addr, size_t cap) {
    ...
    size_t total = 0;
    while (total < cap) {
        if (!VirtualQuery((const void*)a, &mbi, sizeof mbi))
            break;
        if (mbi.State != MEM_COMMIT)
            break;
        DWORD prot = mbi.Protect & 0xFF;
...
template <typename T> inline bool write(uintptr_t addr, T val) {
    if (!writable((const void*)addr, sizeof(T)))
        return false;
    if (__builtin_setjmp(g_guard_jmp)) {
        g_guard_on = 0;
        return false;
