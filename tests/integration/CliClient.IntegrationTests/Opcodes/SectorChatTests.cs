...
public sealed class SectorChatTests
{
    ...
    /// <summary>
    /// Wave 194 missing-arg ERROR literal for case-'f' /ftype.
    /// Matcher at PlayerConnection.cpp:6406 -- NO-GUARD-ELSE-IF
    /// pattern chained off /fradius at 6401, inside the
    /// OUTER-BLOCK-DEV-guard at 6394. THIRD case-'f' user-tier pin --
    /// case-'f' now TRIPLE-PINNED.
    /// </summary>
    private const string MissingArgFtypeLiteral = "Missing arg for option ftype";

    /// <summary>
    /// Wave 194 sibling-arm-pinning hardening (+0 ratchet): pins the
    /// 32-byte wire-shape of the single 0x001D MESSAGE_STRING reply to
    /// user-tier slash <c>/ftype</c> (NO param). Wave 194 deepens
    /// case-'f' to TRIPLE-PINNED.
    ///
    /// <para>
    /// ELSE-IF at 6406 `MatchOptWithParam("ftype", pch, param,
    /// msg_sent)` -- chained off /fradius at 6401 inside the
    /// OUTER-BLOCK-DEV-guard at 6394. Preceding /fradius at 6401
    /// strncmp("fradius","ftype",5) byte 1 'r' vs 't' MISMATCH ->
    /// silent FALSE. /ftype at 6406 matches arg="ftype" -- emits
    /// "Missing arg for option ftype" at 4548 COLOR=5, returns FALSE.
    /// Subsequent else-if /flevel /fcount /faddasteroidtype/
    /// /faddoretofield /fdelorefromfield /faddoretosector/
    /// /fdelorefromsector all MISMATCH. NET RESULT: ONE emit.
    /// </para>
    ///
    /// <para>Budget: 90s.</para>
    /// </summary>
    ...
    /// <summary>
    /// Wave 195 missing-arg ERROR literal for case-'f' /flevel.
    /// Matcher at PlayerConnection.cpp:6411 -- NO-GUARD-ELSE-IF
    /// chained off /ftype at 6406 inside OUTER-BLOCK-DEV-guard at
    /// 6394. FOURTH case-'f' user-tier pin -- case-'f' user-tier now
    /// QUADRUPLE-PINNED.
    /// </summary>
    private const string MissingArgFlevelLiteral = "Missing arg for option flevel";

    /// <summary>
    /// Wave 195 sibling-arm-pinning hardening (+0 ratchet): pins the
    /// 33-byte wire-shape of the single 0x001D MESSAGE_STRING reply to
    /// user-tier slash <c>/flevel</c> (NO param). Wave 195 deepens
    /// case-'f' to QUADRUPLE-PINNED.
    ///
    /// <para>
    /// ELSE-IF at 6411 chained off /ftype at 6406 inside the
    /// OUTER-BLOCK-DEV-guard at 6394. /fradius byte 1 'r' vs 'l'
    /// MISMATCH -> silent FALSE; /ftype byte 1 't' vs 'l' MISMATCH ->
    /// silent FALSE; /flevel matches arg="flevel" -- emits "Missing
    /// arg for option flevel". Subsequent else-if /fcount /faddX all
    /// MISMATCH. NET RESULT: ONE emit.
    /// </para>
    ///
    /// <para>Budget: 90s.</para>
    /// </summary>
    ...
    /// <summary>
    /// Wave 196 missing-arg ERROR literal for case-'f' /fcount.
    /// Matcher at PlayerConnection.cpp:6416 -- NO-GUARD-ELSE-IF
    /// chained off /flevel at 6411 inside OUTER-BLOCK-DEV-guard.
    /// FIFTH case-'f' user-tier pin -- case-'f' QUINTUPLE-PINNED.
    /// </summary>
    private const string MissingArgFcountLiteral = "Missing arg for option fcount";

    /// <summary>
    /// Wave 196 sibling-arm-pinning hardening (+0 ratchet): pins the
    /// 33-byte wire-shape of the single 0x001D MESSAGE_STRING reply to
    /// user-tier slash <c>/fcount</c> (NO param). Wave 196 deepens
    /// case-'f' to QUINTUPLE-PINNED.
    ///
    /// <para>
    /// ELSE-IF at 6416 chained off /flevel at 6411 inside the
    /// OUTER-BLOCK-DEV-guard at 6394. /fhelp byte 1 'h' vs 'c'
    /// MISMATCH; /fradius byte 1 'r' vs 'c' MISMATCH; /ftype byte 1
    /// 't' vs 'c' MISMATCH; /flevel byte 1 'l' vs 'c' MISMATCH;
    /// /fcount matches -- emits "Missing arg for option fcount".
    /// Subsequent else-if /faddasteroidtype /faddoretofield/
    /// /fdelorefromfield /faddoretosector /fdelorefromsector all
    /// MISMATCH at byte 1. NET RESULT: ONE emit.
    /// </para>
    ///
    /// <para>Budget: 90s.</para>
    /// </summary>
    ...
    /// <summary>
    /// Wave 198 missing-arg ERROR literal for case-'f' /faddoretofield.
    /// Matcher at PlayerConnection.cpp:6426 -- NO-GUARD-ELSE-IF chained
    /// off /faddasteroidtype at 6421 inside OUTER-BLOCK-DEV-guard.
    /// SEVENTH case-'f' user-tier pin -- case-'f' SEPTUPLE-PINNED.
    /// </summary>
    private const string MissingArgFaddoretofieldLiteral = "Missing arg for option faddoretofield";

    /// <summary>
    /// Wave 198 sibling-arm-pinning hardening (+0 ratchet): pins the
    /// 41-byte wire-shape of the single 0x001D MESSAGE_STRING reply to
    /// user-tier slash <c>/faddoretofield</c> (NO param). Wave 198
    /// deepens case-'f' to SEPTUPLE-PINNED.
    ///
    /// <para>
    /// ELSE-IF at 6426 chained off /faddasteroidtype at 6421.
    /// /faddasteroidtype strncmp("faddasteroidtype","faddoretofield",16)
    /// byte 4 'a' vs 'o' MISMATCH -> silent FALSE; /faddoretofield
    /// matches arg="faddoretofield" -- emits "Missing arg for option
    /// faddoretofield". Subsequent else-if /fdelorefromfield byte 1
    /// 'd' vs 'a' MISMATCH; /faddoretosector strncmp 15B byte 11 'f'
    /// vs 's'? Actually "faddoretofield" vs "faddoretosector" --
    /// strncmp(option="faddoretosector",arg="faddoretofield",15) byte
    /// 10 's' vs 'f' MISMATCH -> silent FALSE; /fdelorefromsector byte
    /// 1 'd' vs 'a' MISMATCH. NET RESULT: ONE emit.
    /// </para>
    ///
    /// <para>Budget: 90s.</para>
    /// </summary>
    ...
    /// <summary>
    /// Wave 199 missing-arg ERROR literal for case-'f' /fdelorefromfield.
    /// Matcher at PlayerConnection.cpp:6431 -- NO-GUARD-ELSE-IF chained
    /// off /faddoretofield at 6426 inside OUTER-BLOCK-DEV-guard.
    /// EIGHTH case-'f' user-tier pin -- case-'f' OCTUPLE-PINNED.
    /// </summary>
    private const string MissingArgFdelorefromfieldLiteral = "Missing arg for option fdelorefromfield";

    /// <summary>
    /// Wave 199 sibling-arm-pinning hardening (+0 ratchet): pins the
    /// 43-byte wire-shape of the single 0x001D MESSAGE_STRING reply to
    /// user-tier slash <c>/fdelorefromfield</c> (NO param). Wave 199
    /// deepens case-'f' to OCTUPLE-PINNED. SECOND 16-byte option-name
    /// width pin (after /faddasteroidtype).
    ///
    /// <para>
    /// ELSE-IF at 6431 chained off /faddoretofield at 6426. All
    /// preceding f-arms MISMATCH at byte 1 ('h'/'r'/'t'/'l'/'c'/'a' vs
    /// 'd'). /fdelorefromfield matches arg="fdelorefromfield" -- emits
    /// "Missing arg for option fdelorefromfield". Subsequent else-if
    /// /faddoretosector byte 1 'a' vs 'd' MISMATCH; /fdelorefromsector
    /// strncmp 18B vs arg 16B with len_arg<len_opt: arg byte 16=NUL
    /// vs option byte 16='s' MISMATCH -> silent FALSE. NET RESULT: ONE
    /// emit.
    /// </para>
    ///
    /// <para>Budget: 90s.</para>
    ...
    /// <summary>
    /// Wave 201 missing-arg ERROR literal for case-'f' /fdelorefromsector.
    /// Matcher at PlayerConnection.cpp:6441 -- NO-GUARD-ELSE-IF chained
    /// off /faddoretosector at 6436 inside OUTER-BLOCK-DEV-guard. TENTH
    /// case-'f' user-tier pin -- case-'f' DECUPLE-PINNED. Fully exhausts
    /// OUTER-BLOCK-DEV-guard arm coverage on case-'f'.
    /// </summary>
    private const string MissingArgFdelorefromsectorLiteral = "Missing arg for option fdelorefromsector";

    /// <summary>
    /// Wave 201 sibling-arm-pinning hardening (+0 ratchet): pins the
    /// 44-byte wire-shape of the single 0x001D MESSAGE_STRING reply to
    /// user-tier slash <c>/fdelorefromsector</c> (NO param). Wave 201
    /// deepens case-'f' to DECUPLE-PINNED, fully exhausting all ten
    /// OUTER-BLOCK-DEV-guard arms in case-'f'.
    ///
    /// <para>
    /// ELSE-IF at 6441 chained off /faddoretosector at 6436. Preceding
    /// f-arms MISMATCH at byte 1 ('h'/'r'/'t'/'l'/'c'). /faddasteroidtype
    /// byte 1 'a' vs 'd' wait -- arg is "fdelorefromsector", byte 1='d'
    /// vs /faddasteroidtype byte 1='a' MISMATCH; /faddoretofield byte 1
    /// 'd' vs 'a' MISMATCH; /fdelorefromfield strncmp 16B vs arg 17B:
    /// matches "fdelorefrom" (11 bytes), byte 11 arg='s' vs opt='f'
    /// MISMATCH; /faddoretosector byte 1 'd' vs 'a' MISMATCH;
    /// /fdelorefromsector at 6441 matches arg="fdelorefromsector" --
    /// emits "Missing arg for option fdelorefromsector" at 4548 COLOR=5.
    /// No further f-arms. case-'f' breaks. NET RESULT: ONE emit.
    /// </para>
    ///
    /// <para>Budget: 90s.</para>
    /// </summary>
    ...
    /// <summary>
    /// Wave 203 missing-arg ERROR literal for user-tier case-'m' /music.
    /// Matcher at PlayerConnection.cpp:6835 -- SAME-LINE-AND-GUARD-DEV
    /// (AdminLevel() >= 80) else-if chained off /mobaggro at 6830. THIRD
    /// user-tier case-'m' pin -- case-'m' now TRIPLE-PINNED.
    /// </summary>
    private const string MissingArgMusicLiteral = "Missing arg for option music";

    /// <summary>
    /// Wave 203 sibling-arm-pinning hardening (+0 ratchet): pins the
    /// 32-byte wire-shape of the single 0x001D MESSAGE_STRING reply to
    /// user-tier slash <c>/music</c> (NO param). Wave 203 deepens case-'m'
    /// to TRIPLE-PINNED. SAME-LINE-AND-GUARD-DEV with explicit numeric
    /// literal (>= 80) is now DOUBLE-pinned within case-'m'.
    ///
    /// <para>
    /// ELSE-IF at 6835 chained off /mobaggro at 6830. /move strncmp 4B vs
    /// arg 5B with len_arg>len_opt: matches "m" (1 byte), byte 1 arg='u'
    /// vs opt='o' MISMATCH -> silent FALSE; /mobaggro strncmp 8B vs arg
    /// 5B with len_arg<len_opt: byte 1 arg='u' vs opt='o' MISMATCH ->
    /// silent FALSE; /music at 6835 MatchOptWithParam matches arg="music"
    /// -- emits "Missing arg for option music" at 4548 COLOR=5;
    /// AdminLevel() guard skipped due to short-circuit FALSE on LHS. No
    /// further f-arms in case-'m'. case-'m' breaks. NET RESULT: ONE emit.
    /// </para>
    ///
    /// <para>Budget: 90s.</para>
    /// </summary>
    ...
    /// <summary>
    /// Wave 204 missing-arg ERROR literal for user-tier case-'a' /altname.
    /// Matcher at PlayerConnection.cpp:5480 -- GUARD-FIRST-DEV (AdminLevel
    /// >= DEV THEN MatchOptWithParam). FIRST user-tier case-'a' pin --
    /// case-'a' OPENED. GUARD-FIRST-DEV is a NEW pattern distinct from
    /// SAME-LINE-AND-GUARD-DEV (which evaluates matcher first).
    /// </summary>
    private const string MissingArgAltnameLiteral = "Missing arg for option altname";

    /// <summary>
    /// Wave 204 sibling-arm-pinning hardening (+0 ratchet): pins the
    /// 34-byte wire-shape of the single 0x001D MESSAGE_STRING reply to
    /// user-tier slash <c>/altname</c> (NO param). Wave 204 OPENS
    /// user-tier case-'a'. NEW pattern: GUARD-FIRST-DEV (AdminLevel
    /// guard precedes MatchOptWithParam call) -- distinct from
    /// SAME-LINE-AND-GUARD-DEV (Waves 202-203) where matcher runs first.
    ///
    /// <para>
    /// IF at 5480 chained off /altweapon at 5461 within the case-'a'
    /// outer block at 5444. /anon strcmp 5B byte 1 'l' vs 'n' MISMATCH;
    /// /authlevel strcmp 10B byte 1 'l' vs 'u' MISMATCH; /altweapon
    /// GUARD-FIRST passes (status=100 >= DEV=80), MatchOptWithParam
    /// strncmp 9B vs arg 7B with len_arg&lt;len_opt: matches "alt"
    /// (3 bytes), byte 3 arg='n' vs opt='w' MISMATCH -> silent FALSE;
    /// /altname at 5480 GUARD-FIRST passes, MatchOptWithParam matches
    /// arg="altname" -- emits "Missing arg for option altname" at 4548
    /// COLOR=5; /addbaseore GUARD-FIRST passes, MatchOptWithParam
    /// strncmp 10B vs arg 7B byte 1 'l' vs 'd' MISMATCH -> silent FALSE.
    /// case-'a' breaks. NET RESULT: ONE emit.
    /// </para>
    ///
    /// <para>Budget: 90s.</para>
    /// </summary>
    ...
    /// <summary>
    /// Wave 206 missing-arg ERROR literal for user-tier case-'a' /addbaseore.
    /// Matcher at PlayerConnection.cpp:5500 -- GUARD-FIRST-DEV. THIRD
    /// user-tier case-'a' pin -- case-'a' now TRIPLE-PINNED. GUARD-FIRST-DEV
    /// pattern now TRIPLE-pinned within case-'a' (altname + altweapon +
    /// addbaseore), fully exhausting all three DEV-guarded MatchOptWithParam
    /// arms in case-'a'.
    /// </summary>
    private const string MissingArgAddbaseoreLiteral = "Missing arg for option addbaseore";

    /// <summary>
    /// Wave 206 sibling-arm-pinning hardening (+0 ratchet): pins the
    /// 37-byte wire-shape of the single 0x001D MESSAGE_STRING reply to
    /// user-tier slash <c>/addbaseore</c> (NO param). Wave 206 deepens
    /// case-'a' to TRIPLE-PINNED, fully exhausting all three GUARD-FIRST-DEV
    /// arms in case-'a' (altweapon at 5461, altname at 5480, addbaseore at
    /// 5500). GUARD-FIRST-DEV pattern now TRIPLE-pinned within case-'a'.
    ///
    /// <para>
    /// IF at 5500 chained off /altname at 5480 within case-'a' outer block.
    /// /anon strcmp 5B byte 1 'd' vs 'n' MISMATCH; /authlevel strcmp 10B
    /// byte 1 'd' vs 'u' MISMATCH; /altweapon GUARD-FIRST-DEV passes,
    /// MatchOptWithParam strncmp 9B byte 1 'd' vs 'l' MISMATCH -> silent
    /// FALSE; /altname GUARD-FIRST-DEV passes, MatchOptWithParam strncmp
    /// 7B byte 1 'd' vs 'l' MISMATCH -> silent FALSE; /addbaseore at 5500
    /// GUARD-FIRST-DEV passes (status=100 >= DEV=80), MatchOptWithParam
    /// matches arg="addbaseore" -- emits "Missing arg for option addbaseore"
    /// at 4548 COLOR=5. No further arms in case-'a'. case-'a' breaks. NET
    /// RESULT: ONE emit.
    /// </para>
    ///
    /// <para>Budget: 90s.</para>
    ...
    /// <summary>
    /// Wave 207 sibling-arm-pinning hardening (+0 ratchet) literal anchor for
    /// the 25-byte ASCII body "Missing arg for option be" that the server
    /// emits when user-tier slash <c>/be</c> arrives with no argument.
    /// Matcher at PlayerConnection.cpp:5510 -- SAME-LINE-AND-GUARD-BETA. FIRST
    /// 2-byte-option-width pin for case-'b' (the prior case-'b' pins /buff
    /// and /basset are 4B/6B; this is the narrowest option seen so far in
    /// case-'b'). case-'b' is now TRIPLE-PINNED.
    /// </summary>
    private const string MissingArgBeLiteral = "Missing arg for option be";

    /// <summary>
    /// Wave 207 sibling-arm-pinning hardening (+0 ratchet): pins the
    /// 29-byte wire-shape of the single 0x001D MESSAGE_STRING reply to
    /// user-tier slash <c>/be</c> (NO param). Wave 207 deepens case-'b' to
    /// TRIPLE-PINNED. SAME-LINE-AND-GUARD-BETA pattern now SECOND-pinned in
    /// case-'b' (alongside the prior /buff and /basset which are SAME-LINE
    /// patterns but with DEV/GM guards). /be is the FIRST case-'b' pin
    /// against the BETA tier (status=100 >= BETA=30 satisfies).
    ///
    /// <para>
    /// IF at 5510 is the FIRST case-'b' arm. MatchOptWithParam matches
    /// arg="be" (2-byte option width -- joins /ht 5027 as the SECOND
    /// 2-byte option pin overall, confirming structural consistency across
    /// case letters at the narrowest option width). AdminLevel() >= BETA
    /// guard passes (cli_test status=100 >= BETA=30). param is NULL so
    /// no ChatSendChannel emit. allowNoParams=false (default) so the ERROR
    /// branch at 4548 fires: "Missing arg for option be" COLOR=5. NET
    /// RESULT: ONE emit.
    /// </para>
    ///
    /// <para>Budget: 90s.</para>
    ...
    /// <summary>
    /// Wave 243 sibling-arm-pinning hardening (+0 ratchet) literal anchor for
    /// the 59-byte ASCII body "Unable to access selected object - could not
    /// find object ID" that the server emits when user-tier single-slash
    /// <c>/face</c> arrives on a fresh char with no selected target.
    /// Matcher at PlayerConnection.cpp:6360 (arm 7 of case-'f' user-tier
    /// opened at 6280) -- HANDLER-NULL-OBJECT-FALLBACK pattern. FIRST
    /// pin on case-'f' user-tier; case-'f' promoted from UNPINNED to
    /// SINGLE-PINNED. FIRST handler-side NULL-target fallback pin in the
    /// HandleSlashCommands catalogue -- distinct from inner-AdminLevel-
    /// guard pins (Waves 241/242) and from MatchOptWithParam ERROR-fork
    /// pins (Wave 100+ series).
    /// </summary>
    private const string FaceNoTargetLiteral = "Unable to access selected object - could not find object ID";

    /// <summary>
    /// Wave 243 sibling-arm-pinning hardening (+0 ratchet): pins the 63-byte
    /// wire-shape of the single 0x001D MESSAGE_STRING reply to user-tier
    /// single-slash <c>/face</c> on a fresh cli_test character with no
    /// selected target (ShipIndex()-&gt;SetTargetGameID(-1) at
    /// PlayerClass.cpp:1085 is set during post-login). FIRST case-'f'
    /// user-tier pin and FIRST handler-side NULL-target fallback pin.
    ///
    /// <para>
    /// User-tier outer gate at 5434 admits. Switch at 5442 jumps on
    /// *pch='f' -> case-'f' opens at 6280. case-'f' walk for pch="face":
    /// block A `if (AdminLevel() &gt;= GM)` at 6281 admits (cli_test=100):
    ///   arm 1 `MatchOptWithParam("form", pch, ...) &amp;&amp; GM` at 6283:
    ///     strncmp("form","face",4) byte 1 'o' vs 'a' MISMATCH -> returns
    ///     FALSE silently. LHS=false short-circuits.
    ///   arm 2 `if strcmp("flushinv", pch)` at 6291: byte 1 'l' vs 'a'
    ///     MISMATCH. Skip.
    ///   arm 3 `else if strcmp("factionset", pch)` at 6309: bytes 0-2
    ///     match ("fac"), byte 3 't' vs 'e' MISMATCH. Skip.
    ///   arm 4 `else if strcmp("factionoverride", pch)` at 6313: byte 3
    ///     't' vs 'e' MISMATCH. Skip.
    /// block B `if (AdminLevel() &gt;= GM)` at 6337 admits:
    ///   arm 5 `if strcmp("fetch", pch)` at 6339: byte 1 'e' vs 'a'
    ///     MISMATCH. Skip.
    ///   arm 6 `if MatchOptWithParam("find", pch, ...)` at 6345: strncmp
    ///     byte 1 'i' vs 'a' MISMATCH SILENT.
    /// block C (no AdminLevel guard) at 6360:
    ///   arm 7 `if strcmp("face", pch)` at 6360: 4B vs 4B FULL match ->
    ///     `HandleFaceRequest(ShipIndex()-&gt;GetTargetGameID())` at
    ///     PlayerConnection.cpp:8453. Target=-1 (post-login default) ->
    ///     `om-&gt;GetObjectFromID(-1)` returns NULL -> else-branch at
    ///     8466-8469 fires: `SendVaMessage("Unable to access selected
    ///     object - could not find object ID")` COLOR=5 default.
    ///     success=false; msg_sent=true (set at 6363 after handler
    ///     returns).
    ///   arm 8 `else if strcmp("faceme", pch)` at 6365: byte 4 NUL vs 'm'
    ///     MISMATCH. Skip.
    ///   arm 9 `else if strcmp("fgps", pch)` at 6377: byte 1 'g' vs 'a'
    ///     MISMATCH. Skip.
    ///   arm 10 `else if strcmp("fireweapon", pch)` at 6383: byte 1 'i'
    ///     vs 'a' MISMATCH. Skip.
    /// block D `if (AdminLevel() &gt;= DEV)` at 6394 admits:
    ///   arm 11+ `MatchOptWithParam("fhelp"/"fradius"/"ftype"/"flevel"/
    ///     "fcount"/"faddasteroidtype"/...)`: all strncmp byte 1 or byte 2
    ///     MISMATCH -> SILENT-false.
    /// case-'f' breaks at 6447. NET RESULT: ONE emit.
    /// </para>
    ///
    /// <para>
    /// FIRST handler-side NULL-target fallback pin. Catches regression
    /// where (a) HandleFaceRequest's NULL-object else-branch literal is
    /// trimmed or normalized, (b) the post-login default target-id
    /// SetTargetGameID(-1) at PlayerClass.cpp:1085 is changed to a value
    /// that incidentally resolves (e.g. 0 with a magic-zero broker
    /// object), or (c) the case-'f' arm 7 dispatch path is reordered or
    /// gated behind an AdminLevel guard (cli_test=100 would still match,
    /// but a fresh non-admin would silently get no reply -- this pin
    /// keeps the dispatch tier exactly as the real server has it).
    /// SECOND 'long ASCII body' pin after Wave 240 //destroyobject's
    /// 62-byte literal -- 59 bytes is a new emit-body length not pinned
    /// in any prior wave.
    /// </para>
    ///
    /// <para>
    /// Server-integrity (POSITIVE per CLAUDE.md). The /face arm is a
    /// retail-faithful slash-command dispatch; the NULL-target fallback
    /// is the natural error-emit path the real server's HandleFaceRequest
    /// already takes. Pinning preserves the wire shape exactly; no
    /// permissiveness is added.
    /// </para>
    ///
    /// <para>Budget: 90s.</para>
    /// </summary>
    ...
    /// <summary>
    /// Wave 248 sibling-arm-pinning hardening (+0 ratchet) literal anchor for
    /// the 22-byte ASCII body "Mission debugging off." that the server emits
    /// when user-tier single-slash <c>/debugmissions off</c> arrives.
    /// Matcher at PlayerConnection.cpp:6021 (arm 9 of case-'d' user-tier
    /// opened at 5902); the param-bearing dispatch admits, then the inner
    /// <c>strcasecmp(setting, "on") == 0</c> evaluates FALSE (any non-"on"
    /// setting, including "off"), and the else-branch at
    /// PlayerConnection.cpp:6031-6035 emits the body via SendVaMessageC with
    /// EXPLICIT COLOR=12. Wave 246 pinned the if-branch literal ("Mission
    /// debugging activated."); Wave 248 pins the else-branch literal.
    /// </summary>
    private const string MissionDebuggingOffLiteral = "Mission debugging off.";

    /// <summary>
    /// Wave 248 sibling-arm-pinning hardening (+0 ratchet): pins the 26-byte
    /// wire-shape of the single 0x001D MESSAGE_STRING reply to user-tier
    /// single-slash <c>/debugmissions off</c> on a fresh cli_test character.
    /// FOURTH case-'d' user-tier pin (after Waves 174/241/246) -- case-'d'
    /// user-tier promoted from TRIPLE-PINNED to QUADRUPLE-PINNED. THIRD
    /// COLOR=12 emit pin in the entire HandleSlashCommands catalogue (after
    /// Wave 245 /factionoverride strcmp body-emit COLOR=12 and Wave 246
    /// /debugmissions on MatchOptWithParam-with-param strcasecmp-TRUE-branch
    /// COLOR=12). FIRST pin on the strcasecmp-FALSE branch of the
    /// debugmissions MatchOptWithParam body -- PAIRS with Wave 246 to pin
    /// BOTH branches of the same inner-conditional emit. ONE-HUNDRED-SEVENTH
    /// overall byte-exact dispatch pin. Assert.Equal pins the full 26-byte
    /// response shape including COLOR=12 byte.
    ///
    /// <para>
    /// User-tier outer gate at 5434 admits. Switch at 5442 jumps on
    /// *pch='d' -&gt; case-'d' user-tier opens at 5902. arms 1-3 (
    /// MatchOptWithParam("d") / strcmp("don") / strcmp("doff")) -- all
    /// silent or mismatch on pch="debugmissions off". arms 4-8 (dwho /
    /// dialog / debug / deco / dockp) -- all silent or strcmp mismatch.
    /// arm 9 else-if MatchOptWithParam("debugmissions", pch=
    /// "debugmissions off") strncmp 13B match; arg[13]=' ' -&gt; param=
    /// "off", returns TRUE. Body executes: setting=strtok_s("off"," ",...)
    /// ="off"; `setting != NULL` admits; `strcasecmp(setting, "on") == 0`
    /// at PlayerConnection.cpp:6026 evaluates FALSE (strcasecmp returns
    /// nonzero for "off" vs "on"); else-branch at 6031-6035 fires:
    /// m_DebugMissions=true (NOTE: same mutation as the if-branch -- a
    /// preserved retail-source detail, not a test bug); SendVaMessageC(12,
    /// "Mission debugging off.") at 6034. msg_sent=true success=true.
    /// case-'d' breaks. NET: ONE emit (COLOR=12). Wire: `[u16 LE 23][u8 12]
    /// [22B ASCII "Mission debugging off."][NUL]` = 26 bytes.
    /// </para>
    ///
    /// <para>
    /// Regression coverage -- 3 NEW classes prior waves are structurally
    /// blind to: (a) FIRST `strcasecmp("on", ...) == 0` FALSE-branch implicit
    /// pin at PlayerConnection.cpp:6031-6035 -- Wave 246 pinned the
    /// TRUE-branch literal; Wave 248 pins the FALSE-branch literal. A
    /// regression that swapped the two literals (e.g. a typo'd if/else
    /// inversion or both branches emitting the same string) would silently
    /// pass Wave 246 alone; the PAIR catches by failing on this side; (b)
    /// FIRST OFF-state dispatch pin via param-bearing MatchOptWithParam --
    /// the param value "off" exercises the deterministic-else dispatch
    /// path. Catches regression where strcasecmp is replaced with `!strcmp`
    /// (would invert match) or with case-sensitive `strcmp` (would
    /// case-mismatch on "OFF" but still match on "off") -- only the PAIR
    /// distinguishes branch-swap from branch-merge regressions; (c) THIRD
    /// COLOR=12 emit pin overall -- HandleSlashCommands COLOR=12 coverage
    /// promoted from DOUBLE-PINNED (Wave 245 strcmp body-emit + Wave 246
    /// MatchOptWithParam if-branch) to TRIPLE-PINNED (Wave 248 adds
    /// MatchOptWithParam else-branch). A regression that drops COLOR=12
    /// on the else-branch SendVaMessageC call site specifically (while
    /// preserving it on the if-branch) is structurally invisible to Waves
    /// 245/246 alone.
    /// </para>
    ///
    /// <para>
    /// Server-integrity (POSITIVE per CLAUDE.md). No server permissiveness
    /// added. The /debugmissions off arm is a retail-faithful slash-command
    /// dispatch; sending the "off" param exercises the natural FALSE-branch
    /// admit path; the COLOR=12 emit is the genuine server response.
    ...
    public async Task SlashDebugmissionsOff_OnAdminAccount_PinsExactReplyWireShape()
    {
        ...
        // COLOR=12 via SendVaMessageC -- THIRD COLOR=12 pin after Waves
        // 245/246.
    ...
    /// arrives with no argument. Matcher at PlayerConnection.cpp:4892
    /// (case-'e' arm of the admin-tier switch opened at 4724) -- NO-INLINE-
    /// GUARD pattern (outer GM gate at 4716 is the only AdminLevel check
    /// on the dispatch path). FIRST admin-tier case-'e' pin; case-'e'
    /// admin-tier now SINGLE-PINNED.
    /// </summary>
    private const string MissingArgEditfactionLiteral = "Missing arg for option editfaction";

    /// <summary>
    /// Wave 212 sibling-arm-pinning hardening (+0 ratchet): pins the
    /// 38-byte wire-shape of the single 0x001D MESSAGE_STRING reply to
    /// admin-tier double-slash <c>//editfaction</c> (NO param). Wave 212
    /// opens admin-tier case-'e' to SINGLE-PINNED status.
    ///
    /// <para>
    /// Outer gate at 4716 (<c>Msg[0]=='/' &amp;&amp; Msg[1]=='/' &amp;&amp;
    /// AdminLevel() >= GM</c>) admits the dispatch; cli_test status=100
    /// satisfies GM=50. Switch at 4724 jumps on *pch -> case-'e' opens at
    /// 4890. case-'e' has two arms (BOTH plain `if`, not else-if):
    /// MatchOptWithParam("editfaction", pch) at 4892 matches 11B vs 11B.
    /// NO inline AdminLevel guard. param=NULL so the body branch is not
    /// reached; allowNoParams=false (default) fires the 4548 ERROR fork:
    /// "Missing arg for option editfaction" COLOR=5. The second arm
    /// MatchOptWithParam("editplayerfaction", pch="editfaction") -- 17B
    /// vs 11B; strncmp byte 4 'p' vs 'f' MISMATCH -> silent FALSE. case-'e'
    /// breaks. NET RESULT: ONE emit.
    /// </para>
    ///
    /// <para>
    /// First admin-tier case-'e' MatchOptWithParam ERROR-fork pin. FIFTH
    /// admin-tier NO-INLINE-GUARD pin (joins //halloween 4927,
    /// //restartcomms 5055, //countsp 4831, //displayplayerfaction 4865).
    /// FIRST admin-tier pin where the case body uses two PLAIN `if`s
    /// (rather than else-if chain) -- catches regression where a
    /// maintainer collapses the structure into else-if and accidentally
    /// changes single-emit semantics if both options ever happened to
    /// match (they cannot today, but the regression coverage pins the
    /// invariant against future arm additions).
    /// </para>
    ///
    /// <para>Budget: 90s.</para>
    /// </summary>
