...
public static class TestedOpcodes
{
    ...
    /// </summary>
    public const int MinTestedCount = 27;
    ...
    public static readonly IReadOnlyList<TestedOpcode> Opcodes = new[]
    {
        ...
        new TestedOpcode(0x0057, "SKILL_UP",
            "Opcodes/SectorSkillUpTests.cs SkillUp_OnUninitialisedSkillSlot_DoesNotBreakConnection_RequestTimeStillRoundTrips — client sends a canonical 10-byte SkillAction payload {int32 GameID=0; int SkillPoints=0; int16 SkillID=169} on the sector connection after STAGE handshake completes. SkillID=169 is the last valid index into the 170-slot _Skills.Skill[] array (server/src/AuxClasses/AuxSkills.h:23-26) and is never trained for any profession (AuxSkill::Init at AuxSkill.h:39-47 zeroes Level and MaxSkillLevel), so Player::HandleSkillAction (server/src/PlayerSkills.cpp:97-106) trips the very first early-return guard (SkillLevel == SkillMaxLevel where 0==0). No DB write, no AuxPlayer/AuxShip refresh, no direct reply — the retail server doesn't emit one on this branch either, and CLAUDE.md forbids fabricating. Test is a survival probe: send SKILL_UP, send REQUEST_TIME, assert CLIENT_SET_TIME echoes our sentinel tick. Catches: (a) PacketStructures.h SkillAction long-revert regression (currently `{int32_t GameID; int SkillPoints; short SkillID}` = 10B canonical; if anyone changes `int SkillPoints` to `long SkillPoints` the struct grows to 14B on Linux x86_64 — SkillID would read from byte 12 past end of 10B payload into undefined memory, then index a random Skill[] slot, potentially tripping into UpgradeSkill/SaveNewSkillLevel and corrupting player state in the DB); (b) proxy default-case ForwardClientOpcode dropping the opcode (0x0057 is not explicitly listed in proxy/ClientToServer_linux_stubs.cpp); (c) Skill[SkillID].GetMaxSkillLevel() reading past the 170-array bound if a future refactor shrinks the array without updating callers; (d) m_Mutex deadlock if a regression moves the lock above the early-return guards. Per CLAUDE.md server-integrity: the wire shape is exactly what retail Win32 client emits when user clicks the \"+\" on a skill in the skill tree."),
