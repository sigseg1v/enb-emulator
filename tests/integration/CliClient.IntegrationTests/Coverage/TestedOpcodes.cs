...
public static class TestedOpcodes
{
    ...
    /// </summary>
    public const int MinTestedCount = 30;
    ...
    public static readonly IReadOnlyList<TestedOpcode> Opcodes = new[]
    {
        ...
        new TestedOpcode(0x0019, "SET_TARGET",
            "Opcodes/SectorRequestTargetTests.cs RequestTarget_OnNullTarget_ReceivesSetTargetWithSentinelTargetIdMinusOne — received as the server's reply to 0x0017 REQUEST_TARGET. Player::SendSetTarget (server/src/PlayerConnection.cpp:3663) builds the 8-byte SetTarget struct {int32 GameID; int32 TargetID} and emits it via SendOpcode through the standard UDP fan-out path (same path as Wave 8's MESSAGE_STRING and other server-originated frames). Test asserts payload size is exactly 8B (would catch any wire-size revert of the SetTarget struct) and that the TargetID field equals -1 (the hard-coded sentinel from SendSetTarget itself — not echoed from the wire, so a regression that mirrored back the request's TargetID instead of using the literal -1 would be caught here)."),
