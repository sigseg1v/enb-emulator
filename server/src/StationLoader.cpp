...
StationLoader::~StationLoader()
{
	...
	for (mapStations::iterator itrOList = m_StationList.begin(); itrOList != m_StationList.end(); ++itrOList) 
		delete itrOList->second;
	m_StationList.clear();
	for (mapNPCs::iterator itrNList = m_NPCList.begin(); itrNList != m_NPCList.end(); ++itrNList) 
	{
		if (itrNList->second)
		{
			for (mapNodes::iterator itrCList = itrNList->second->NPCInteraction.talk_tree.Nodes.begin(); itrCList != itrNList->second->NPCInteraction.talk_tree.Nodes.end(); ++itrCList)
			{
				if (itrCList->second)
				{
			...
			itrNList->second->NPCInteraction.talk_tree.Nodes.clear();
			delete itrNList->second;
		}
	}
	m_NPCList.clear();
	for (mapRooms::iterator itrOList = m_RoomList.begin(); itrOList != m_RoomList.end(); ++itrOList) 
		delete itrOList->second;
	m_RoomList.clear();
	for (mapTerminals::iterator itrOList = m_TermList.begin(); itrOList != m_TermList.end(); ++itrOList) 
		delete itrOList->second;
	m_TermList.clear();
	for (mapItems::iterator itrOList = m_ItemList.begin(); itrOList != m_ItemList.end(); ++itrOList) 
		delete itrOList->second;
	m_ItemList.clear();
}
...
bool StationLoader::LoadStations()
{
	...
	LogMessage("Loading Stations from SQL (%d)\n", StationsinDB);
	...
	for(int x=0;x<StationsinDB;x++)
	{
		...
		if (current_station)
		{
			...
			if (welcome) strcpy_s(current_station->WelcomeMessage, sizeof(current_station->WelcomeMessage), welcome);
            current_station->NumTerms = 0;
            current_station->NumNPCs = 0;
			current_station->JTLevel = 0;
...
void StationLoader::AddTerminals(StationTemplate *current_station, long room_id, sql_connection_c *connection)
{
    ...
    for(int yt=0;yt<Term_result.n_rows();yt++)
    {
        ...
        if(current_term)
        {
			...
			long level = TermData["terminal_level"];
			if (level > 0 && current_term->Type == 3)
			{
				current_station->JTLevel = level;
			}
        }
    }
}
...
void StationLoader::AddNPCs(StationTemplate *current_station, long room_id, sql_connection_c *connection)
{
    ...
    for(int y=0;y<NPCs_result.n_rows();y++)
    {
        ...
        if(current_npc)
        {
            ...
            strcpy_s(current_npc->Avatar.avatar_first_name, sizeof(current_npc->Avatar.avatar_first_name), 
				NPCData["first_name"]);
            strcpy_s(current_npc->Avatar.avatar_last_name, sizeof(current_npc->Avatar.avatar_last_name),
				NPCData["last_name"]);
			...
			strcpy_s(m_TalkTreeBuffer, m_TalkTreeBufferLen, NPCData["talk_tree_handle"]);
			m_TalkTreeBuffer[m_TalkTreeBufferLen-1] = '\0';
