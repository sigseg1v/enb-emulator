...
void MOBSpawn::PopulateSpawn(bool live)
{
    ...
    for (short spawn = 0; spawn < m_SpawnCount; spawn++)
    {
        ...
        {  
			...
			mob->SetRespawnTime(m_Respawn_time);
			mob->SetRandomAttackRange();

			if (m_FirstMOBID == 0)
			{
				m_FirstMOBID = mob->GameID();
			}
        }
    }
}
...
void MOBSpawn::AddGroupHate(long GameID, long Hate)
{
	// Add to hate list if required

	if (!m_GroupAttack) return;

	//go through each spawn member and add some hate

	MOB *mob;
	ObjectManager *om = GetObjectManager();

	for (int count = 0; count < m_SpawnCount; count++)
	{
		mob = (MOB*) om->GetObjectFromID(m_FirstMOBID + count);

		if (mob)
		{
			mob->AddHate(GameID, Hate);
		}
	}
}