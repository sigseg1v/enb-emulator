...
MissionParser::~MissionParser(void)
{
    for (MissionList::iterator itrMList = m_MissionList.begin(); itrMList < m_MissionList.end(); ++itrMList) 
		delete *itrMList;
    m_MissionList.clear();
}
					...
					if (current_node)
					{
						if (m_CurrentID == XML_TAG_ID_STAGE)
						{
							...
							else
							{
                                ...
                                strcpy_s(current_node->description, length+1, attrib);
								current_node->description[length] = '\0';
							}
						}
						else if (m_CurrentID == XML_TAG_ID_REWARDS)
						{
							...
							else if (current_node->reward)
							{
                                ...
                                strcpy_s(current_node->reward->description, length + 1, attrib);
								current_node->reward->description[length] = '\0';
							}
                            else
                            {
						...
						if (m_CurrentID == XML_TAG_ID_TALKTREE)
						{
                            ...
                            { //got branch destination, now read text
                                ...
                                else
                                {
                                    ...
                                    strcpy_s(this_desc->branch_text, length + 1, attrib);
									this_desc->branch_text[length] = '\0';
