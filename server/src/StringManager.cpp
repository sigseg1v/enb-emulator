...
StringManager::StringManager()
{
	...
	g_cumulative_mem += m_CurrentSize;
	LogMessage("Allocated 0x%08x for string manager\n", m_CurrentSize);

    // Set the very first byte to be the null string
    *m_Memory = 0;
    m_CurLoc = m_Memory + 1;

    LogMessage("String manager initialized\n");
}
...
char * StringManager::AddStr(unsigned long Hash, char *Str)
{
	...
	strcpy_s(m_CurLoc, STRING_MEMORY_SIZE - (m_CurLoc - m_Memory), Str);
...
void StringManager::Statistics(char * CopyBuffer, int BufferSize)
{
    char buffer[256];
    ...
    sprintf_s(buffer, sizeof(buffer),
		"StringManager Statistics: %d of %d hash locations used. %d bytes used. %d bytes left.",
		Count, m_HashTable.Size, Used, Left);

    if (CopyBuffer != 0)
    {
        strcpy_s(CopyBuffer, BufferSize, buffer);
    }
    else
    {
        LogMessage("%s\n",buffer);
