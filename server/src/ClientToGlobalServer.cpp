...
void Connection::ProcessGlobalServerOpcode(short opcode, short bytes)
{
    ...
    default:
        LogMessage("%sProcessGlobalServerOpcode -- UNRECOGNIZED OPCODE 0x%04x\n", GetLogInfo(), opcode);
        break;
    }
}
...
void Connection::HandleGlobalConnect()
{
	...
	bool account_in_use = g_PlayerMgr->CheckAccountInUse(m_AccountUsername);
	if (!account_in_use) account_in_use = g_ServerMgr->m_ConnectionMgr.CheckAccountInUse(m_AccountUsername, this);
...
void Connection::HandleDeleteCharacter()
{
	...
	LogMessage("%sDelete character %d\n", GetLogInfo(), avatar_id);
...
void Connection::HandleCreateCharacter()
{
    ...
    LogMessage("%sNew character `%s` created\n", GetLogInfo(), create->avatar.avatar_first_name);
...
void Connection::HandleGlobalTicketRequest()
{
    ...
    if (avatar_id == -1)
    {
        LogMessage("%sGlobalTicketRequest() Error obtaining global character slot id\n", GetLogInfo());
        return;
    }
    ...
    if (player)
    {
        ...
        LogMessage("%sAllocated playerID of 0x%08x [%d]\n", GetLogInfo(), player->GameID(), player->CharacterID());
    ...
    {
		LogMessage("%s** Critical Error: Can't get AvatarID [%d]! **\n", GetLogInfo(), avatar_id);
...
void Connection::ProcessProxyGlobalOpcode(short opcode, short bytes)
{
	...
	case ENB_OPCODE_2002_TICKET:
		LogMessage("%sProcessing TCP Ticket.\n", GetLogInfo());
    ...
    case ENB_OPCODE_2004_AVATARLOGIN:
        LogMessage("%sReceived TCP AvatarLogin confirm (start player)\n", GetLogInfo());
        HandleGlobalTicketRequest();
        break;

    case ENB_OPCODE_200B_CREATE_AVATAR:
        LogMessage("%sReceived TCP Create Avatar packet\n", GetLogInfo());
        HandleCreateCharacter();
        break;

    case ENB_OPCODE_200D_DELETE_AVATAR:
        LogMessage("%sReceived TCP Delete Avatar packet\n", GetLogInfo());
		HandleDeleteCharacter();
        //HandleAvatarDeleteRequest(msg, hdr, source_addr, source_port);
        break;

	case ENB_OPCODE_2008_MASTER_HANDOFF:
		LogMessage("%sRecieved TCP master handoff\n", GetLogInfo());
		HandleMasterHandoff();
		break;

	default:
		LogMessage("%sIllegal opcode from Server to Global Account connection\n", GetLogInfo());
		break;
	}
}
...
void Connection::HandleAccountValid(short bytes)
{
    ...
    LogMessage("%sAccount info received for: %s\n", GetLogInfo(), username);
...
void Connection::ProcessTicketInfo(short bytes)
{
	...
	if (!account_in_use) account_in_use = g_ServerMgr->m_ConnectionMgr.CheckAccountInUse(account_name, this);
