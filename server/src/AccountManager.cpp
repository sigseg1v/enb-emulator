...
void AccountManager::SetupTickets()
{
    ...
    bool AccountManager::ReadDatabase(CharacterDatabase *database, long avatar_id)
    {
        ...
        sprintf_s(database->info.location, sizeof(database->info.location), "%s", g_ServerMgr->GetSectorName(sector_id));
	    ...
	    strcpy_s(database->ship_data.ship_name, sizeof(database->ship_data.ship_name), account_row["name"]);
...
char * AccountManager::GetUsernameFromTicket(char *ticket)
{
	...
	*pTerminate = 0;
	...
	char *pName = g_StringMgr->GetStr(ticket);

	return pName;

	//is ticket expiry a big issue?
    /*m_Mutex.Lock();
...
long AccountManager::CreateCharacter(GlobalCreateCharacter * create)
{
    ...
    sprintf_s(database.info.location, sizeof(database.info.location), "%s", g_ServerMgr->GetSectorName(start_sector));
...
void AccountManager::BuildAvatarList(GlobalAvatarList * list, long account_id)
{
    ...
    for (int i=0; i<5; i++)
    {
        ...
        {
            sprintf_s(database.info.location, sizeof(database.info.location), "%s", 
