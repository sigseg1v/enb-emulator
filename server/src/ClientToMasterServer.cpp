...
void Connection::ProcessMasterServerOpcode(short opcode, short bytes)
{
	...
	default :
        LogMessage("%sProcessMasterServerOpcode -- UNRECOGNIZED OPCODE 0x%04x\n", GetLogInfo(), opcode);
		break;
	}
}
...
void Connection::HandleMasterJoin()
{
	...
	LogMessage("%sReceived MasterJoin packet, ToSectorID = %d\n", GetLogInfo(), sector_id);
...
void Connection::SendServerRedirect(long sector_id)
{
	...
	if (g_ServerMgr->m_SectorServerMgr.LookupSectorServer(redirect))
	{
		...
		LogMessage("%sMaster Server sending ServerRedirect packet, SectorID = %d\n", GetLogInfo(), sector_id);
		SendResponse(ENB_OPCODE_0036_SERVER_REDIRECT, (unsigned char *) &redirect, sizeof(redirect));
	}
	else
	{
		LogMessage("%sUnable to redirect client to a working server\n", GetLogInfo());
		// TODO: GlobalError();
	}
}
