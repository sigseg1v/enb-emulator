    ...
    if (p)
    {
        ...
        if (*p == '=')
        {
            ...
            if (*p == '"')
            {
                p++;
                char temp[512];
                strncpy_s(temp, sizeof(temp), p, sizeof(temp));
                temp[511] = 0;
                if (strtok_s(temp, "\"", &next_token))
                {
                    if (strlen(temp) < length)
                    {
                        strcpy_s(value, length, temp);
                        success = true;
                    }
                }
            }
        ...
        *dest = '\0';
        if ((src[0] == '<') && (src[1] == '/'))
        {
            *buffer = src;
            ...
            strncpy_s(data, length, temp, length);
            data[length-1] = 0;
            delete [] temp;
        }
...
char * XmlParser::EncodeXmlString(char * str)
{
    ...
    if (temp_string)
    {
        ...
        if (result)
        {
            strcpy_s(result, strlen(temp_string) + 1, temp_string);
			result[strlen(temp_string)] = '\0';
        }
        delete [] temp_string;
    }
...
bool XmlParser::ParseFloatArray(char *attrib_values, int count, float *array)
{
    ...
    char *p = strtok_s(attrib_values, ",", &next_token);
