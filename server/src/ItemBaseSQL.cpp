...
bool ItemBaseParser::LoadItemBase(ItemBase ** GlobalDB)
{
	...
	for(int item=0;item<ItemList_result->n_rows();item++)
	{
		...
		sprintf_s(QueryString, sizeof(QueryString), "SELECT * FROM `item_manufacture` WHERE `item_id` = '%d'", ItemID);
		// Execute Query
		...
		switch(m_Data->ItemType)
		{
			...
			case 14:
				{
					sprintf_s(QueryString, sizeof(QueryString), "SELECT * FROM `item_beam` WHERE `item_id` = '%d'", ItemID);
					// Execute Query
					Item_result = SqlQuery(&connection, QueryString);
					if (Item_result && Item_result->n_rows() > 0)
					{
			...
			case 16:
				{
					sprintf_s(QueryString, sizeof(QueryString), 
						"SELECT * FROM `item_projectile` Inner Join `item_ammo_type` ON `item_projectile`.`ammo_type_id` = `item_ammo_type`.`id` WHERE `item_projectile`.`item_id` =  '%d'", ItemID);
					// Execute Query
					Item_result = SqlQuery(&connection, QueryString);
					if (Item_result && Item_result->n_rows() > 0)
					{
			...
			case 15:
				{
					sprintf_s(QueryString, sizeof(QueryString),
						"SELECT * FROM `item_missile` Inner Join `item_ammo_type` ON `item_missile`.`ammo_type_id` = `item_ammo_type`.`id` WHERE `item_missile`.`item_id` =  '%d'", ItemID);
					// Execute Query
					Item_result = SqlQuery(&connection, QueryString);
					if (Item_result && Item_result->n_rows() > 0)
					{
			...
			case 11:
				{
					sprintf_s(QueryString, sizeof(QueryString),
						"SELECT a.*,b._Range,b.EnergyUse FROM item_device as a LEFT JOIN item_effect_container AS b ON a.item_id=b.itemid WHERE a.item_id =  '%d'", ItemID);
					// Execute Query
					Item_result = SqlQuery(&connection, QueryString);
					if (Item_result && Item_result->n_rows() > 0)
					{
			...
			case 7:
				{
					sprintf_s(QueryString, sizeof(QueryString), 
						"SELECT * FROM `item_reactor` WHERE `item_reactor`.`item_id` =  '%d'", ItemID);
					// Execute Query
					Item_result = SqlQuery(&connection, QueryString);
					if (Item_result && Item_result->n_rows() > 0)
					{
			...
			case 6:
				{
					sprintf_s(QueryString, sizeof(QueryString), 
						"SELECT * FROM `item_engine` WHERE `item_engine`.`item_id` =  '%d'", ItemID);
					// Execute Query
					Item_result = SqlQuery(&connection, QueryString);
					if (Item_result && Item_result->n_rows() > 0)
					{
			...
			case 2:
				{
					sprintf_s(QueryString, sizeof(QueryString),
						"SELECT * FROM `item_shield` WHERE `item_shield`.`item_id` =  '%d'", ItemID);
					// Execute Query
					Item_result = SqlQuery(&connection, QueryString);
					if (Item_result && Item_result->n_rows() > 0)
					{
			...
			case 10:
				{
					sprintf_s(QueryString, sizeof(QueryString), 
						"SELECT * FROM `item_ammo` Inner Join `item_ammo_type` ON `item_ammo`.`ammo_type_id` = `item_ammo_type`.`id` WHERE `item_ammo`.`item_id` =  '%d'", ItemID);
					// Execute Query
					Item_result = SqlQuery(&connection, QueryString);
					if (Item_result && Item_result->n_rows() > 0)
					{
		...
		sprintf_s(QueryString, sizeof(QueryString), "SELECT * FROM item_effect_container WHERE `ItemID` = '%d'", ItemID);
		// Execute Query
		...
		if (Effect_Container_result && Effect_Container_result->n_rows() > 0)
		{
			...
			sprintf_s(QueryString, sizeof(QueryString), "SELECT * FROM item_effects Inner Join item_effect_base ON item_effects.item_effect_base_id = item_effect_base.EffectID WHERE `ItemID` = '%d'", ItemID);
			// Execute Query
