...
struct Quat{
	...
	Quat( float x, float y, float z, float w):w(w),v(x,y,z){
	}
	Quat operator-()const{
		return Quat( w,-v );
	}
	...
	EulerToQuat (float heading, float attitude, float bank) const {    // Assuming the angles are in radians.
	...
	Vector QuatToEuler() const {
		Vector EulerAngle;
		float test = v.x*v.y + v.z*w;
		if (test > 0.499) { // singularity at north pole
			EulerAngle.z = 2.0f * atan2f(v.x,w);
			EulerAngle.x = PI/2;
			EulerAngle.y = 0.0f;
			return EulerAngle;
		...
		EulerAngle.z = atan2f(2.0f*v.y*w-2*v.x*v.z , 1.0f - 2.0f*sqy - 2.0f*sqz);
		EulerAngle.x = asinf(2.0f*test);
		EulerAngle.y = atan2f(2.0f*v.x*w-2*v.y*v.z , 1.0f - 2.0f*sqx - 2.0f*sqz);
