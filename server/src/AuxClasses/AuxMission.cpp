...
{
	ReplaceString(Data->Name, NewName, 1, sizeof(Data->Name));
}

void AuxMission::SetSummary(char * NewSummary)
{
	ReplaceString(Data->Summary, NewSummary, 2, sizeof(Data->Summary));
}

void AuxMission::SetReward(char * NewReward)
{
	ReplaceString(Data->Reward, NewReward, 3, 128);
}

void AuxMission::SetFailureConsequence(char * NewFailureConsequence)
{
	ReplaceString(Data->FailureConsequence, NewFailureConsequence, 4, 128);
}

void AuxMission::SetIssuingFaction(char * NewIssuingFaction)
{
	ReplaceString(Data->IssuingFaction, NewIssuingFaction, 5, 64);
}
