...
void AuxMobIndex::BuildCreatePacket(unsigned char *buffer, long &index)
{
	...
	*/
	...
	char FlagIndex1 = 0x20;  //by default send only max hull pts.

	if (Data.MaxHullPoints > 0.0f) //always send hull at first in case this mob just respawned
	{
		FlagIndex1 |= 0x10; //send hull pts to show hull damage
	}

	if (Data.IsOrganic) //always send shield at first in case this mob just respawned
	{
		FlagIndex1 |= 0x0C; //send shields & max shields to show Organic damage
	}
	...
	AddData(buffer, char(ExtendedFlags[1] & FlagIndex1), index);  // Indexes 9, and potentially 6 & 7 or 8 (to show hull damage without needing to click)
	...
	if (buffer[8] & 0x10)	//Index 8 - sent if hull is damaged for damage FX
	{
		AddData(buffer,Data.HullPoints,index);
	}
...
void AuxMobIndex::SetData(_MobIndex *NewData)
{
	...
	ReplaceString(Data.Owner, NewData->Owner, 1, 64);
...
{
	ReplaceString(Data.Owner, NewOwner, 1, 64);
}

void AuxMobIndex::SetTitle(char *NewTitle)
{
	ReplaceString(Data.Title, NewTitle, 2, 64);
}

void AuxMobIndex::SetRank(char *NewRank)
{
	ReplaceString(Data.Rank, NewRank, 3, 64);
}
...
{
	ReplaceString(Data.InterruptibleAbilityName, NewInterruptibleAbilityName, 53, 128);
}
...
{
	ReplaceString(Data.FactionIdentifier, NewFactionIdentifier, 57, 64);
}
