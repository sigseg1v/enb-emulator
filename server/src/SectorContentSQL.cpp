...
SectorContentParser::~SectorContentParser()
{
	for (SectorDataMap::iterator itrAList = m_SectorList.begin(); itrAList != m_SectorList.end(); ++itrAList) 
	{
		if (itrAList->second)
		{
			for (SectorOreList::iterator itrOList = itrAList->second->OreList.begin(); itrOList < itrAList->second->OreList.end(); ++itrOList) 
				delete *itrOList;
			itrAList->second->OreList.clear();
			delete itrAList->second->obj_manager;
			delete itrAList->second;
		}
	}
	m_SectorList.clear();
	for (SystemDataMap::iterator itrAList = m_SystemList.begin(); itrAList != m_SystemList.end(); ++itrAList) 
		delete itrAList->second;
	m_SystemList.clear();
	for (AsteroidContentList::iterator itrAList = m_AsteroidContentList.begin(); itrAList != m_AsteroidContentList.end(); ++itrAList) 
		delete itrAList->second;
	m_AsteroidContentList.clear();
}
...
bool SectorContentParser::ParseSectorContent(long parse_id)
{
    ...
    LogMessage("Loading Sectors from SQL (%d)", (int)Sector_result->n_rows());
	...
	for(int x=0;x<Sector_result->n_rows();x++)
	{
		...
		current_sector->obj_manager->SetSectorID(sector_id);
		g_ServerMgr->SetObjectManMap(sector_id, current_sector->obj_manager);
		...
		current_sector->challenge_rating = (int)SectorSQLData["challenge_rating"];
		...
		if (greetings)
		{
			sprintf_s(current_sector->greetings, 128, "%s", greetings);
		}
		...
		for(int y=0;y<Object_result->n_rows();y++)
		{
			...
			if (current_object == 0)
			{
				...
				case 1:
					current_object = current_sector->obj_manager->AddNewObject(OT_MOB, true);  //used for turrets only for now, use for other stationary MOBs maybe
					break;

				case 41:	// Gravity Well
					current_object = current_sector->obj_manager->AddNewObject(OT_GWELL);
					current_object->SetPositionType(POSITION_CONSTANT);
					break;

				case 40:	// Radiation
					current_object = current_sector->obj_manager->AddNewObject(OT_RADIATION);
					current_object->SetPositionType(POSITION_CONSTANT);
					break;

				case 42:	// Turrets and stationary MOBs
			...
			long sound_effect = (long)ObjectData["sound_effect_id"];

			if (sound_effect > 0)
			{
				current_object->SetBroadcastID(sound_effect);
			}
            ...
            case OT_FIELD:
				...
				if (new_object)
				{
					...
					if (FldResult.n_rows() != 0)
					{
						sql_row_c FieldData;
						FldResult.fetch_row(&FieldData);
						local_respawn = (int)FieldData["local_respawn_time"];
					}
					...
					if  (local_respawn > 0)
					{
						current_object->SetRespawnTimer(local_respawn);
					}
					else
					{
						current_object->SetRespawnTimer((int)ObjectData["respawn_timer"]);
					}
            ...
            case OT_MOBSPAWN:
				if (new_object)
				{
					...
					current_object->SetRespawnTime(ObjectData["respawn_time"]);
					current_object->PopulateSpawn(false);
					mobspawn = (MOBSpawn*)current_object;
					mobspawn->SetRespawnTime(ObjectData["respawn_time"]);
					mobspawn->SetGroupAttack(ObjectData["group_aggro"]);
				}
                break;
			...
			case OT_MOB: //turret
				...
				treat_as_mob = (int)ObjectData["treat_as_mob"] != 0 ? true : false;
				if (treat_as_mob)
				{
					turret->SetDefaultStats(0, DRIFT, 0, 50);
				}
				else
				{
				...
				turret->AddBehaviourPosition(turret->Position());
			...
			case OT_STARGATE:
				...
				current_object->SetSecurityLvl(ObjectData["minSecurityLevel"]);
				if (((int)ObjectData["classSpecific"]) == 1)
				{
					current_object->SetClassSpecific();
				}
				ProcessDefaultObjectStats(current_object, ObjectData);
				if (current_object->CreateType() == 10) //found a local stargate
				{
					current_object->SetLocalStargate();
					current_object->SetCreateType(11);
				}
			...
			case OT_RADIATION:
				if (current_object->NavType() > 0)
                {
                    current_object->SetObjectType(current_object->ObjectType());
                }
				ProcessDefaultObjectStats(current_object, ObjectData);
				break;
...
void SectorContentParser::LoadSystems(sql_connection_c *connection)
{
    ...
    strcpy_s(QueryString, sizeof(QueryString), "SELECT * FROM `systems`");
...
void SectorContentParser::LoadAsteroidContentSelection(sql_connection_c *connection)
{
	...
	for (int i = 1; i <= RESOURCE_TYPE_SIZE; i++)
	{
		...
		if (m_AsteroidContentList.find(i) != m_AsteroidContentList.end())
		{
