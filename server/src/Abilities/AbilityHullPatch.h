...
**
*/
#ifndef _ABILITY_HULLPATCH_H_INCLUDED_
#define _ABILITY_HULLPATCH_H_INCLUDED_

#include "AbilityBase.h"

class AHullPatch : public AbilityBase
{
public:
	AHullPatch(CMob *me) : AbilityBase(me, STAT_SKILL_HULL_PATCH) {}
public:
...
private:
	...
	float CalculateHullRepair ( float SkillLevel, long SkillRank );
	float CalculateAOEHullRepair ( float SkillLevel, long SkillRank );
	long DetermineSkillRank(int SkillID);	
	void ProximityAOE(CMob *target, short seq, proxparam repair, proxparam p2, proxparam p3);
	void RechargeFriendly(CMob *Target, float ChargeAmount);
