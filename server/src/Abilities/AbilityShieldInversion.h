...
**
*/
#ifndef _ABILITY_SHIELD_INVERSION_H_INCLUDED_
#define _ABILITY_SHIELD_INVERSION_H_INCLUDED_

#include "AbilityBase.h"

class AShieldInversion : public AbilityBase
{
public:
	AShieldInversion(CMob *me);
public:
...
private:
	float CalculateShieldDrain ( float SkillLevel, long SkillRank );
	...
	bool RequiresTarget();
	void ProximityAOE(CMob *target, short seq, proxparam tag, proxparam effect, proxparam p3);
	float m_Conversion;
	float m_ShieldDrain;
	float m_DamageTick;
	float m_AdjustedSkillLevel;
	bool  m_hit_something;
	long m_FirstEffectID;
	long m_CurrentEffectID;
};
