...
**
*/
#ifndef _ABILITY_ASELFDESTRUCT_H_INCLUDED_
#define _ABILITY_ASELFDESTRUCT_H_INCLUDED_

#include "AbilityBase.h"

class ASelfDestruct : public AbilityBase
{
public:
	ASelfDestruct(CMob *me);
public:
...
private:
	...
	float CalculateDamage ( float SkillLevel, long SkillRank );
	long  DetermineSkillRank(int SkillID);
	void ProximityAOE(CMob *target, short seq, proxparam tag, proxparam damage, proxparam p3);
	bool IsUsedOnEnemies()  { return false; };
	bool IsUsedOnFriends()  { return false; };
