...
**
*/
#ifndef _ABILITY_AREPAIREQUIPMENT_H_INCLUDED_
#define _ABILITY_AREPAIREQUIPMENT_H_INCLUDED_

#include "AbilityBase.h"

struct equipment_damage
{
	Player* player;
	float equipID[20];
	float total_damage;
...
class ARepairEquipment : public AbilityBase
{
public:
	ARepairEquipment(CMob *me) : AbilityBase(me, STAT_SKILL_REPAIR_EQUIPMENT) {}
public:
...
private:
	...
	float CalculateDamage ( Player* p, equipment_damage* damage );
	float RepairDamage ( float repair_rate, equipment_damage* damage );
	int   RepairTarget( Player* p , float repair_rate );
};
