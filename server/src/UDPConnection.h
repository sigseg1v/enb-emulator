...
typedef std::vector<long> IP_vec;
...
private:
	...
	void				HandleLoginStageAck(char *msg, EnbUdpHeader *hdr, const long source_addr, const short source_port);

	//Net7SSL methods
	void				HandleSSLregister(char *buffer, long IPaddr, short port);
	void				HandleSSLLogin(char *buffer, long addr, short port);

	void				HandlePlayerCountRQ(char *buffer, long addr, short port);
