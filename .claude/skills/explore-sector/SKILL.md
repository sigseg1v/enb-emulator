...
| `scripts/rescue.sh {detect\|tow-xy\|rescue}` | DETERMINISTIC wreck-recovery state machine. `detect` -> `WRECKED`/`ALIVE` by fuzzy-matching the amber `<<Distress` HUD label (independent of every other UI element). `tow-xy` -> window-rel `x y` of the "I need a tow" comm reply (fuzzy phrase match). `rescue` (default) -> full loop: close the map ONCE if open (it occludes the reply), click "I need a tow", wait for `<<Distress` to clear. `drive.py check_wreck/handle_wreck` call it; it is checked EVERY poll the ship isn't moving (speed 0 or absolute distance frozen). |
| `scripts/undock.sh` | launch the ship back into space when DOCKED at a station (prints `UNDOCKED`/`IN-SPACE`/`FAILED`). See "Undocking from a station" below. `handle_wreck` calls it after a tow drops us in a bay. |
