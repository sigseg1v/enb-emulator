...
SSL_Connection * SSL_Connection::ReSetSSL_Connection(SOCKET s, unsigned long *ip_addr, SSL_CTX *ssl_context)
{
...
char *SSL_Connection::GetResponse(char *recv_buffer, size_t *response_length)
{
	...
	*response_length = 0;
    ...
    {
		//TODO: send 'who' request to server
...
char *SSL_Connection::AuthLogin(size_t *response_length, char *recv_buffer)
{
	...
	*/
	...
	if (username && password)
	{
		...
		// check accounts from here

		// TODO: This is a good place to check for "banned" IP addresses.
		// send message to server to check this is a valid account
	...
	if (username && port && max_sectors && version)
	{
		...
		if (strcmp(version, expected_version) == 0)
		{
			...
			if ((port_number >= 3500) && (max_sectors > 0))
			{
				...
				{
					LogMessage("RegisterSectorServer for %s, IP address %s, port %d\n",
						username, g_InternalIP, port_number);
...
SSL_DenyList::SSL_DenyList()
{
	//load the denylist database
	...
	sprintf_s(query_str, sizeof(query_str), "SELECT * FROM `ssl_deny_list`");

	ssl_query.run_query(query_str);
    ssl_query.store(&ssl_result);

	for(int i=0;i<ssl_result.n_rows();i++)
	{
		ssl_result.fetch_row(&ssl_row);
		char *ssl_addr_c = ssl_row["deny_addr"];
		unsigned long ssl_addr = inet_addr(ssl_addr_c);
		m_AddrList.push_back(ssl_addr);
	}
