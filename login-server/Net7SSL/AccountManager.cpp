...
//sql_connection_c g_SQL_Data_Conn;
...
bool AccountManager::GetEmailAddress(char *username, char *buffer, int buflen)
{
	...
	strcpy_s(buffer,buflen,account_row[0]);
	return true;
}
...
long AccountManager::ValidateAccount(char *username, char *password)
{
	...
	sprintf_s(query_str, sizeof(query_str), 
		"SELECT `id` FROM `accounts` WHERE `username` = '%s' AND `password` = MD5('%s')", username, password);
...
bool AccountManager::AddUser(char *username, char *password, char *access)
{
	...
	char pass[128];
	_snprintf_s(pass, 128, 128, "MD5('%s')", password);

	account_buider.SetTable("accounts");
	account_buider.AddData("username", username);
	account_buider.AddData("password", pass);
...
void AccountManager::DeleteCharacter(long player_id)
{
	sql_query_c account_query (&m_SQL_Conn);
	char query_str[256];

	//Delete this character from the tables
	sprintf_s(query_str, sizeof(query_str), "DELETE FROM `ship_info` WHERE `avatar_id` = '%d'", player_id);
	account_query.run_query(query_str);

	sprintf_s(query_str, sizeof(query_str), "DELETE FROM `ship_data` WHERE `avatar_id` = '%d'", player_id);
	account_query.run_query(query_str);

	sprintf_s(query_str, sizeof(query_str), "DELETE FROM `avatar_data` WHERE `avatar_id` = '%d'", player_id);
	account_query.run_query(query_str);

	sprintf_s(query_str, sizeof(query_str), "DELETE FROM `avatar_info` WHERE `avatar_id` = '%d'", player_id);
	account_query.run_query(query_str);


	//remove position info
	sprintf_s(query_str, sizeof(query_str), "DELETE FROM `avatar_position` WHERE `avatar_id` = '%d'", player_id);
	account_query.run_query(query_str);

	sprintf_s(query_str, sizeof(query_str), "DELETE FROM `avatar_level_info` WHERE `avatar_id` = '%d'", player_id);
	account_query.run_query(query_str);

	sprintf_s(query_str, sizeof(query_str), "DELETE FROM `avatar_ammo` WHERE `avatar_id` = '%d'", player_id);
	account_query.run_query(query_str);

	sprintf_s(query_str, sizeof(query_str), "DELETE FROM `avatar_inventory_items` WHERE `avatar_id` = '%d'", player_id);
	account_query.run_query(query_str);

	sprintf_s(query_str, sizeof(query_str), "DELETE FROM `avatar_vault_items` WHERE `avatar_id` = '%d'", player_id);
	account_query.run_query(query_str);

	sprintf_s(query_str, sizeof(query_str), "DELETE FROM `avatar_equipment` WHERE `avatar_id` = '%d'", player_id);
	account_query.run_query(query_str);

	sprintf_s(query_str, sizeof(query_str), "DELETE FROM `avatar_mission_progress` WHERE `avatar_id` = '%d'", player_id);
	account_query.run_query(query_str);

	sprintf_s(query_str, sizeof(query_str), "DELETE FROM `missions_completed` WHERE `avatar_id` = '%d'", player_id);
	account_query.run_query(query_str);
	
	sprintf_s(query_str, sizeof(query_str), "DELETE FROM `faction_data` WHERE `avatar_id` = '%d'", player_id);
	account_query.run_query(query_str);
	
	sprintf_s(query_str, sizeof(query_str), "DELETE FROM `avatar_exploration` WHERE `avatar_id` = '%d'", player_id);
	account_query.run_query(query_str);

	sprintf_s(query_str, sizeof(query_str), "DELETE FROM `avatar_skill_levels` WHERE `avatar_id` = '%d'", player_id);
	account_query.run_query(query_str);

	sprintf_s(query_str, sizeof(query_str), "DELETE FROM `avatar_faction_level` WHERE `avatar_id` = '%d'", player_id);
	account_query.run_query(query_str);

	sprintf_s(query_str, sizeof(query_str), "DELETE FROM `avatar_recipes` WHERE `avatar_id` = '%d'", player_id);
	account_query.run_query(query_str);

	sprintf_s(query_str, sizeof(query_str), "DELETE FROM `ignore_lists` WHERE `avatar_id` = '%d'", player_id);
	account_query.run_query(query_str);

	sprintf_s(query_str, sizeof(query_str), "DELETE FROM `friends_lists` WHERE `avatar_id` = '%d'", player_id);
	account_query.run_query(query_str);
}
...
{
	//TODO: save database
	//g_SaveMgr->AddSaveMessage(SAVE_CODE_DATABASE, avatar_id, sizeof(CharacterDatabase), (unsigned char *)database);
	sql_query_c account_query (&m_SQL_Conn);
	char query_str[256];
...
bool AccountManager::ReadDatabase(CharacterDatabase *database, long avatar_id)
{
	...
	sprintf_s(database->info.location, sizeof(database->info.location), "%s", GetSectorName(sector_id));
	...
	strcpy_s(database->avatar.avatar_first_name, sizeof(database->avatar.avatar_first_name), account_row["first_name"]);
	strcpy_s(database->avatar.avatar_last_name, sizeof(database->avatar.avatar_last_name), account_row["last_name"]);
	...
	strncpy(database->ship_data.ship_name, account_row["name"], sizeof(database->ship_data.ship_name));
...
char * AccountManager::BuildTicket(char *username)
{
	...
	time_slot->username[sizeof(time_slot->username)-1] = '\0';
...
void AccountManager::GetUsernameFromTicket(char *ticket, char *name, long length)
{
	unsigned long cur_time = GetNet7TickCount();

	if (!ticket) return;

	char *pTerminate = strchr(ticket, '-');
	*pTerminate = 0;

	strncpy_s(name, length, ticket, length);
}
...
long AccountManager::CreateCharacter(GlobalCreateCharacter * create)
{
	...
	unsigned char chr, last = 0, count = 0;
	...
	if (IsForbidden(create->avatar.avatar_first_name))
	{
		return G_ERROR_RESTRICTED_LIST;
	}

	//Check ship name
	if (IsForbidden(create->ship_data.ship_name))
	{
	...
	sprintf_s(database.info.location, sizeof(database.info.location), "%s", GetSectorName(start_sector));
...
void AccountManager::BuildAvatarList(GlobalAvatarList * list, long account_id)
{
	...
	for (int i=0; i<5; i++)
	{
		...
		{
			sprintf_s(database.info.location, sizeof(database.info.location), "%s", GetSectorName(ntohl(database.info.sector_id)));
	...
	strcpy_s(list->galaxy[0].Name, sizeof(list->galaxy[0].Name), g_Galaxy_Name);
	list->galaxy[0].Name[sizeof(list->galaxy[0].Name)-1] = '\0';
	list->galaxy[0].GalaxyID = 1;
	list->galaxy[0].IP_Address = ntohl(inet_addr(LOCAL_IP));
	list->galaxy[0].port = ntohs(MASTER_SERVER_PORT);
	list->galaxy[0].NumPlayers = g_PlayerCount;
	list->galaxy[0].MaxPlayers = g_MaxPlayerCount;
	list->galaxy[0].unknown2 = 1;
}

long AccountManager::GetPlayerSector(long avatar_id)
{
	long sector_id = 0;
