...
public:
	SSL_Connection * ReSetSSL_Connection(SOCKET s, unsigned long *ip_addr, SSL_CTX *ssl_context);
...
typedef std::vector<unsigned long> SSL_AddressList;
...
public:
	bool CheckSSLAddress(unsigned long address);

private:
	SSL_AddressList m_AddrList;
